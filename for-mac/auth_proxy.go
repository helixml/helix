...
// into every request. This works around WKWebView's iframe cookie isolation:
// instead of relying on the browser to store and send cookies, we inject
// them server-side.
	...
	proxy.Director = func(req *http.Request) {
		...
		if session != "" {
			...
			if csrf != "" {
				...
				// Inject the X-CSRF-Token header for state-changing requests.
				// The browser can't read the helix_csrf cookie because we strip
				// Set-Cookie from responses, so the frontend's CSRF interceptor
				// gets null. We inject the header server-side instead.
