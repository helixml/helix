...
# NOTE: We use "sandbox0" instead of Docker's default "docker0" bridge to prevent
# conflicts when session dockerds clean up their own "docker0" bridges.

# Wait for sandbox0 bridge to have 10.213.0.1 assigned
SANDBOX_GATEWAY="10.213.0.1"
MAX_WAIT=30
for i in $(seq 1 $MAX_WAIT); do
    if ip addr show sandbox0 2>/dev/null | grep -q "$SANDBOX_GATEWAY"; then
        echo "✅ sandbox0 bridge ready with gateway $SANDBOX_GATEWAY"
        break
    fi
    if [ $i -eq $MAX_WAIT ]; then
        echo "❌ Timeout waiting for sandbox0 bridge"
        exit 1
    fi
    sleep 1
done
...
# Start dns-proxy bound to the sandbox0 bridge gateway specifically
