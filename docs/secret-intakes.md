...
For the first proposed website login connector, see the [Meydan customer portal design](../design/2026-09-26-meydan-connector-handoff.md). It scopes the prototype to a personal account and read-only operations. Helix will fill the real portal form in its own isolated browser from a claimed intake; the resulting browser session is used by bounded bot tools.
