...
2. **`tools/` knows about a specific runtime backend.** `tools/hire_worker.go` imports `agent/helix` and `helix/helixclient` directly, and at hire time it conditionally calls `WorkerProject.Ensure(...)` to materialise a Helix project. This is the largest violation of `CLAUDE.md`'s "keep the core generic" rule that I see. The `agent.Spawner` abstraction was supposed to hide which backend is active; instead, hiring is backend-aware. (See `tools/hire_worker.go:12-15`.)
