...
| `agent/helix/project.go:24` `ProjectApplier` | Concrete `helixclient.Client` field (`spawner.go:75`) | Acceptable inside the Runtime context — `helixclient` *is* the Helix runtime. But once `Client` is split per §4, `ProjectApplier` should hold only `ProjectClient + RepoClient + AppClient`, not the whole god interface. |
...
- `agent/helix/project.go:24` `type ProjectApplier struct` — "Applier" is a verb-noun. Replace with `Project` or `HelixProject` once it's clear what it owns.
...
| 2 | Split `helixclient.Client` (`client.go:42-133`) into per-concern sub-interfaces. Keep the concrete `realClient` implementing the union. | ISP §4 row 1, DIP §5 row 4, and shrinks the surface that `chat.HelixBridge` / `ProjectApplier` / `Workspace` each see. | M — one file refactor in `helixclient/`; callers narrow their declared type. |
