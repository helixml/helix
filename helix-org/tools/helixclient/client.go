...
type Client interface {
	...
	// call as StartChat, but additionally returns whether the SSE
	// stream surfaced a transient WS-not-connected error after the
	// session ID came through. Callers use this to drive a poll-retry
	// loop while the desktop boots.
...
// StartChatWithStatus is the streaming-aware variant. It returns the
// resolved Session plus a flag indicating whether the SSE stream
// surfaced a transient "no external agent WebSocket connection" error
// after the session ID was emitted. Callers (the chat bridge) use the
// flag to drive a poll-retry loop that re-sends the user's message
// once the desktop's Zed agent finishes booting and dials home.
//
// Background: Helix's session_handlers.go dispatches the chat command
// synchronously in the same request that creates a new zed_external
// session. The dispatch fails immediately with ErrNoExternalAgentWS
// because the brand-new container hasn't WS-connected yet, and Helix
// then marks the interaction state=error — out of reach of
// pickupWaitingInteraction (which only retries waiting interactions).
// The workaround lives client-side: detect the soft error, wait for
// the container to come up, and re-send the same prompt as a
// follow-up. See helix_bridge.send for the call site.
