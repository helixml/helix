...
- **Interface**: One MCP endpoint and one HTTP diagnostic. `/workers/{id}/mcp` (Streamable HTTP transport, no auth yet) carries every read and mutation of the org graph; the worker ID in the URL identifies the caller, and the server exposes only the tools that worker holds grants for. `/tail` is a plain HTTP long-poll that streams events across all Channels — kept on HTTP because it has no Worker identity. There are no other HTTP endpoints, and there is no jsonapi mutation surface.
- **CLI**: A thin client binary with three subcommands. `helix-org bootstrap` opens the SQLite store directly and seeds the initial owner Worker — the one operation that cannot use MCP, because there is no Worker yet. `helix-org serve` runs the HTTP listener (MCP + `/tail`). `helix-org prompt` and `helix-org tail` connect from the outside: `prompt` spawns Claude Code pointed at a Worker's MCP endpoint, `tail` long-polls `/tail`. After bootstrap, the CLI never opens the database again.
