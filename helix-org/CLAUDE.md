...
- **Storage**: GORM with `AutoMigrate`, against helix's existing Postgres connection in production (H4.4) or a SQLite file in tests / dev wirings that pass non-Postgres helixStore impls. No raw SQL migration files. The org-store interfaces live at `api/pkg/org/store/`; the sqlite impl at `api/pkg/org/store/sqlite/` (H4.1); the Postgres impl at `api/pkg/org/store/postgres/` (H4.3) wraps `(*helixstore.PostgresStore).GormDB()`. The legacy `FILESTORE_TYPE=fs` requirement is gone.
...
- **Owner seeding**: On first start against an empty database, `initHelixOrgHandler` (`api/pkg/server/helix_org.go`) calls `bootstrap.Run` which creates the initial owner Worker (`w-owner`), Position (`p-root`), Role (`r-owner`), Environment, activation Stream (via the shared `tools.EnsureActivationStream` helper — B5.9), and the root grant set. After seeding, all org-graph mutations go through MCP.
- **Activation aggregate**: per-Spawner-invocation `activation.Activation` rows (id `a-…`) at `api/pkg/org/store/sqlite/activation.go` capture `{WorkerID, Triggers, StartedAt, EndedAt, Outcome, TranscriptStreamID}` for the audit / replay surface (B5.4–5.6). Per-Worker coalescing lives in `activation.Queue` (lifted out of the dispatcher in B5.10). The chat bridge persists rows for every owner-chat send (B5.11), and the bridge itself is now Worker-agnostic — the chat target is a config parameter (`HelixConfig.WorkerID`), not a hardcoded owner concept (B5.12). `worker_log` gains an `activationId` filter to read one activation's window (B5.7); `hire_worker` returns the hire-Activation ID synchronously so callers can poll (B5.8).
- **Operational config**: runtime-mutable keys (worker.runtime, transport.postmark, helix.url, …) live in `api/pkg/org/config/` (lifted from helix-org/config in H3). Distinct from `api/pkg/config/` which is static envconfig.
...
- **helixclient package** (`api/pkg/org/helixclient/`) is transitional. After H1.3d every runtime call goes through the canonical ports (`ProjectService`, `SpawnerClient`, `ProjectGit`); helixclient survives only as the HTTP+WS transport adapter satisfying those ports. H1.4 — delete the package outright — is blocked on H1.3c (replace the helixclient adapter with a direct controller adapter); see the deferred-work note at the top of `api/pkg/org/helixclient/client.go`.
- **Auth + multi-tenancy**: helix's `requireUser + requireFeature` middleware gates every surface. The chat bridge now stashes a typed `runtimehelix.HelixIdentity{UserID, OrganizationID, BearerToken}` on every request (H6.1–H6.2), and Workers carry the same `OrganizationID` (H5.1) — stamped by bootstrap on the owner (H5.2) and inherited by hires from the calling Worker (H5.3). The remaining tenant-scoping in queries (H5.4+) and the Activation-row identity stamp (H6.3+) layer on top of this scaffolding.
- **Package layout**: every Go package now lives under `api/pkg/org/` (H8 complete). `helix-org/` itself holds only docs (this file, TODO.md, design/), sample data (demos/), and the Makefile. The Go module surface is single-rooted.
