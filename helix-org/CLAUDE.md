...
- **CLI**: A thin client binary with two subcommands. `helix-org bootstrap` opens the SQLite store directly and seeds the initial owner Worker — the one operation that cannot use MCP, because there is no Worker yet. Pass `--install-claude-mcp` to bootstrap to register the owner's MCP endpoint with the local `claude` CLI; from then on plain `claude` sessions can drive the org. `helix-org serve` runs the HTTP listener. After bootstrap, the CLI never opens the database again — humans drive the org through `claude`, and Workers do the same when activated by the dispatcher.
