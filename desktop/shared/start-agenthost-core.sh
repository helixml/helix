...
start_cursor_helix() {
    ...
    # Prevent duplicate instances
    CURSOR_LOCK_FILE="/tmp/helix-cursor-startup.lock"
    if [ -f "$CURSOR_LOCK_FILE" ]; then
        OLD_PID=$(cat "$CURSOR_LOCK_FILE" 2>/dev/null)
        if [ -n "$OLD_PID" ] && kill -0 "$OLD_PID" 2>/dev/null; then
            echo "Cursor startup already running (PID $OLD_PID) - exiting duplicate"
            exit 0
        fi
        echo "Stale lock file found, removing..."
        rm -f "$CURSOR_LOCK_FILE"
    fi
    echo $$ > "$CURSOR_LOCK_FILE"
    trap 'rm -f "$CURSOR_LOCK_FILE"' EXIT
    ...
    # Step 2: Read folders to open
    # =========================================
    read_editor_folders "Cursor"

    # =========================================
    # Step 3: Configure Cursor API key
    # =========================================
    # CURSOR_API_KEY is set by the CursorBridge via environment
    # The bridge handles authentication and prompt routing
    echo "CURSOR_API_KEY is ${CURSOR_API_KEY:+set}"

    # =========================================
    # Step 4: Launch Cursor with auto-restart
    # =========================================
    # --disable-workspace-trust: Skip the trust dialog for mounted workspaces
    run_editor_restart_loop "Cursor" cursor --disable-workspace-trust "${EDITOR_FOLDERS[@]}"
}
