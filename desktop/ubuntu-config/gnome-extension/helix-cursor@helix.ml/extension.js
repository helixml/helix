...
export default class HelixCursorExtension extends Extension {
  ...
  enable() {
    ...
    try {
      ...
      try {
        ...
        this._cursorTracker = Meta.CursorTracker.get_for_display(
          global.display,
        );
    ...
    this._retryCount = 0;
    this._initialTimeoutId = GLib.timeout_add(
  ...
  _sendInitialCursor() {
    ...
    if (!success && this._retryCount < 30) {
      ...
      this._retryCount++;
      console.log(
        "[HelixCursor] Socket not ready, will retry (" +
          this._retryCount +
          "/30)...",
      );
  ...
  _onCursorChanged(forceResend = false) {
    try {
      ...
      if (width <= 0 || height <= 0) {
        console.log(
          "[HelixCursor] Invalid cursor dimensions: " + width + "x" + height,
        );
        return false;
      }
      ...
      const cursorName = this._getCursorNameFromFingerprint(
        hotspotX,
        hotspotY,
        width,
        height,
      ...
      console.log(
        "[HelixCursor] Cursor changed: hotspot=(" +
          hotspotX +
          "," +
          hotspotY +
          ") -> " +
  ...
  _getCursorNameFromFingerprint(hotspotX, hotspotY, width, height) {
    ...
    console.log(
      "[HelixCursor] No fingerprint match for hotspot (" +
        hotspotX +
        "," +
        hotspotY +
        "), using default",
