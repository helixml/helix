...
|-----------|--------|---------|
| Claude Code setup token | `sk-ant-oat01-` | `sk-ant-oat01-59jYxX8OG76RMOi-...` |
| Anthropic API key | `sk-ant-api03-` | `sk-ant-api03--_4V9f-bhQsZTyc...` |
| OAuth refresh token | `sk-ant-ort01-` | (internal, not user-facing) |

Setup tokens from `claude setup-token` start with `sk-ant-oat01-`. This is the only format that should be accepted in the setup token dialog.
...
**Why prefix-match `sk-ant-oat01-` specifically?** This is the known prefix for setup tokens generated by `claude setup-token`. If Anthropic changes the prefix in the future, we update the validation. The alternative — accepting anything — is the current broken behavior.
