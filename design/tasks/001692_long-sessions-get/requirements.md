...
3. **As a user**, I want older messages to load automatically when I scroll up, so I can review earlier context seamlessly.
...
- By default, only the most recent ~20 interactions are rendered (configurable constant).
- Older interactions load automatically when the user scrolls up (virtual/infinite scroll).
