...
- [~] Add `specTitleFromRequirements(content string) string` helper in `git_http_server.go` (strip leading `#`, trim whitespace, return first non-empty line)
- [ ] In `processDesignDocsForBranch()`, call the helper on `requirements.md` content and update the task `Name` field on every push (not only on the initial `spec_review` transition); skip if title is empty
