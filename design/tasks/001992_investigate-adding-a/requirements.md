...
- **`remote_server` / `HeadlessProject` already runs without UI**, but it is the SSH remote-host daemon. The agent itself runs on the **client** in SSH-remote mode — `language_model::init`, language-model providers, `UserStore`, and `RefreshLlmTokenListener` are all main-`zed`-only (`crates/zed/src/main.rs:657, 663`); `remote_server` initializes none of them. So `remote_server` exposes project state (worktrees, MCP processes) to a Zed client elsewhere and isn't a drop-in agent worker. See design D-2 for the full breakdown of why we don't reuse the binary.
