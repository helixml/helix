...
- [~] In `websocket_external_agent_sync.go`, extract a `buildFullStatePatchEvent(sessionID, owner, interactionID string, entries []wsprotocol.ResponseEntry) ([]byte, error)` helper that builds a full-state `interaction_patch` event (all entries with `patch_offset=0`, no previous entries)
