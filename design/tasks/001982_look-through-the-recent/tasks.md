...
- [~] In `ExternalAgentDesktopViewer.tsx`'s `handleSendMessage` path, call the same helper before `NewInference` (the existing `invalidateQueries` is now consolidated inside the helper)
