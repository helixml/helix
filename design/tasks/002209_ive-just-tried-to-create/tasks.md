...
- [x] E2E of the MCP surface: `TestProjectDiscoveryOverMCP` drives the REAL HTTP MCP server (`server.NewFromStore` → registry → tool → service → port) — a seeded PM bot connects, both tools are advertised, `list_projects` returns the org's projects, `get_project` is org-scoped (unknown id errors). NOTE: a live browser e2e across two projects with real event triggering was NOT run — standing up a helix-org owner bot + two projects + live attention events in the inner instance is disproportionate; the wired server→tool→service→port path is covered by the in-process HTTP MCP test above, and the triggering path by the publisher/filter unit tests.
