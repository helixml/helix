...
- [ ] KNOWN INNER-INSTANCE ISSUE (not this feature): the pm-bot's OWN autonomous worker **desktop** session fails to start Zed's Claude agent with `Custom agent server "claude-acp" is not registered` (repeated retries → error). proj-a's normal desktop with the identical agent config works, so this is a Zed agent-server registration problem in the helix-org worker-desktop provisioning path, independent of the MCP tools (which are proven working via the live MCP endpoint). Worth a separate ticket.
