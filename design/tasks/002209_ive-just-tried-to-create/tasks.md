...
## Live agent confirmation (spec-task work by a real agent)
- [x] Fixed a real misconfig found during live testing: the org-graph worker's agent app (`634a130b…`) was created with **empty `provider`/`model`** → set `provider=anthropic`, `model=claude-opus-4-8` (matching the working onboarding agent) via `PUT /api/v1/apps/{id}`, then restarted the session.
- [x] **End-to-end spec-task work confirmed on the pm-bot's cross-project task** (`spt_01kwktgs…` in proj-a): pm-bot `create_spectask` → a real Claude agent generated the spec (`spec_generation → spec_review`, Zed thread connected) → pm-bot `review_spectask_spec` + `approve_spectask_spec` (MCP) advanced it to **`implementation`** with branch `feature/000001-populate-proj-a-readme` → the implementation agent is running (board "In Progress", live desktop, 0/5 checklist). The "Specs ready for review" attention event also fired (exercises the publisher field-coercion path). Screenshot `03-pm-bot-task-in-implementation.png`.
- [x] **Bot autonomously self-driving — CONFIRMED after fixing the org worker provider config.** The earlier `claude-acp not registered` error was a provider-config issue (as the reviewer expected), in two places: the Helix Org **worker defaults** were `worker.credentials=subscription` with empty `worker.provider`/`worker.model`, and the bot's agent app carried `code_agent_credential_type=subscription`. Fix: set org settings `worker.credentials=api_key` / `worker.provider=anthropic` / `worker.model=claude-opus-4-8`, patched the pm-bot agent app to `code_agent_credential_type=api_key` (matching proj-a's working agent), restarted the session → Zed connected (thread `5d5f0759…`), no error.
  - On wake the pm-bot's **own Claude agent autonomously** called `list_projects`/`list_spectasks`/`get_spectask`/`reports`/`managers`/`read_events` and reported the cross-project task it manages.
  - Given a plain-English instruction, the bot **autonomously created and started** a new spec task in proj-a: `spt_01kwkx4jeneftsygcgtwmme93e` "Bot autonomous e2e test" (it called `create_spectask` + `start_spectask_planning` itself) → reached `spec_generation` (verified in Postgres; board shows it in Planning as #000002, agent running). Screenshot `04-bot-autonomous-task.png`.

## Shepherd to ready-for-PR (bot-driven via chat)
- [x] Drove task `spt_01kwkx4jeneftsygcgtwmme93e` toward PR by **chatting with the bot** (not direct MCP). The bot's own Claude agent: confirmed status via `get_spectask`, called `create_spectask_prs`, then polled `get_spectask`/`read_events`, and verified the pushed branch by `git ls-remote`.
- Result: implementation complete and pushed — branch `feature/000002-verify-org-bot` @ `d46810a` present on the proj-a repo; `create_spectask_prs` accepted without error.
- Status stays `implementation` and NO PR URL because proj-a's repo is the **internal Helix git server** (`/git/code-proj-a-…`), not GitHub. `create_spectask_prs` opens PRs against *external* repos attached to the project; proj-a has none (created as a "New repository" in onboarding). The bot correctly reported this and did not invent a PR link.
- To reach `pull_request` status with a real GitHub PR, the project must be created with an **External GitHub repository** connected (+ GitHub OAuth/app) — a setup choice, not a feature gap.
