...
- [x] Edit `api/pkg/server/server.go` — register the new attention event routes
- [ ] Run `./stack update_openapi` to regenerate API client after adding swagger annotations
- [ ] Write unit tests for `attention_service.go` — deferred to after all trigger points are wired
- [ ] Write unit tests for `store_attention_events.go` — deferred to after all trigger points are wired
