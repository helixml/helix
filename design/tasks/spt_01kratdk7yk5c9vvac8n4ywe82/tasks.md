...
- [ ] Add `CircularProgress` to the MUI imports in `frontend/src/components/spec-tasks/ReviewSubmitDialog.tsx`.
- [ ] Update the primary confirmation `<Button>` to render a `startIcon` of `<CircularProgress size={16} color="inherit" />` when `isSubmitting` is true (else `undefined`).
- [ ] Update the button label so that while `isSubmitting` is true it reads "Approving…" for the approve decision and "Submitting…" for the request-changes decision; restore "Approve" / "Submit Feedback" otherwise.
