...
## Backend
- [ ] Add `AttachmentPaths []string` field to `CreateTaskRequest` and `SpecTask` model in `simple_spec_task.go`
- [ ] In `spec_driven_task_service.go`, when building the planning Interaction: if attachments exist, read images from filestore, base64-encode them, and use `PromptMessageContent` with `ImageURLPart` entries instead of plain `PromptMessage`
