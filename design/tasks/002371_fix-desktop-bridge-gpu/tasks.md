...
- [ ] Add `api/pkg/desktop/gst_pipeline_leak_test.go` with build tag `cgo && linux`, skipping when `/dev/nvidia0` is absent or `nvh264enc` does not register
- [ ] Implement the fd/GPU measurement helpers: count `/dev/nvidia0` symlinks in `/proc/self/fd`; read own-pid GPU MiB from `nvidia-smi --query-compute-apps`
- [ ] Sub-test A: 20 × (`NewGstPipeline` → `Stop()` without `Start()`) — the "never started" shape; assert flat after warm-up
- [ ] Sub-test B: 20 × (`NewGstPipeline` → `Start()` → consume a few frames → `Stop()`) — the "started" shape; assert flat after warm-up
- [ ] Run against unmodified code and capture the failing output for the PR

## Phase 3 — Fix the leak

- [ ] Add a `releaseGObject(*glib.Object)` helper that disarms the go-glib finalizer then unrefs, with a comment explaining that the finalizer lives on the embedded `*glib.Object`
- [ ] Keep the appsink element wrapper on `GstPipeline` so its `TransferFull` ref can be released explicitly
- [ ] Clear the appsink callbacks in `Stop()` so `GDestroyNotify` releases go-gst's global `gopointer` handle (local cgo helper if go-gst v1.4.0 offers no nil-safe form)
- [ ] Drain remaining bus messages in `watchBus` before it returns; flush the bus and release it explicitly in `Stop()`
- [ ] Release the forced realtime clock explicitly instead of only nil-ing `g.realtimeClock`
- [ ] Release the pipeline last, in reverse acquisition order, after `SetState(NULL)` + the state-change wait; delete the incorrect "the GC finalizer releases the other" comment
...
- [ ] Release each sample explicitly in `onNewSample` after the frame bytes are copied
- [ ] Release the probe element in `diagnoseGPUEncoderFailure` (and cache the availability result so a CUDA context is not created on every parse failure)
