...
- [x] Verify the amd64 sandbox pipeline runs the build steps (and skips e2e/push) on the feature branch — build #2397: `build-qwen-code` ✓, `build-zed` ✓, e2e/desktops/sandbox/push skipped, `default` green
- [x] Fix `build-qwen-code` to build only bundle-required workspaces in dependency order (the `--workspaces` approach failed in #2396; corrected and verified green in #2397)
- [ ] Merge to main
