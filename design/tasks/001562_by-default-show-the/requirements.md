...
When a user opens an issue (clicks on its title in the task list), the left panel currently shows the full session chat thread. The first message in that thread is a long, technical blob of agent instructions — it reads like a system prompt but is technically the first user message sent to the agent — with the user's actual request appended at the end. This is confusing and buries the most valuable content.
...
3. The agent instructions portion of the first message to be **collapsed** by default, accessible only if the user is curious
...
**US3 – Agent instructions are accessible but not intrusive**
> As a user who is curious about the agent's instructions, I want to be able to expand a collapsed section to see the full first message, but I don't want it taking up prime real estate by default.
...
- [ ] The agent instructions portion of the first chat message is **collapsed** by default in the session view
- [ ] A clearly labelled collapsible section ("Agent Instructions") allows expanding to view the full first message content
- [ ] The user's actual request (the human-readable part of the first message) remains visible in the chat thread as before
