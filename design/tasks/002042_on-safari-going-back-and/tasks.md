...
- [~] Wire BFCache `pageshow` listener in `WebSocketStream` — register inside `startHeartbeat()` alongside the existing `visibilityHandler`, unregister in `stopHeartbeat()`. On `event.persisted === true`, log and call `this.reconnect()`.
