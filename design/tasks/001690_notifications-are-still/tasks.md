...
- [x] Add `assignee_name?: string` to `AttentionEvent` TypeScript interface in `frontend/src/hooks/useAttentionEvents.ts`
- [x] Add assignee avatar (MUI `Avatar`, 16px, initials) to the left of the timestamp in notification items when `filterMine=false` in `GlobalNotifications.tsx`
