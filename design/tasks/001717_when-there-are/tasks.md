...
- [ ] Cap canvas rendering resolution to device screen size on mobile/tablet — in `renderVideoFrame()` (`websocket-stream.ts:954-956`), cap canvas dimensions to `screen.width * devicePixelRatio` × `screen.height * devicePixelRatio` instead of matching decoded frame size. Use `drawImage(frame, 0, 0, cappedWidth, cappedHeight)` to downscale. Server stream unchanged. iPad Pro (2732×2048) keeps full res; smaller devices save GPU memory (e.g. 4K stream on iPad Air: ~22MB canvas vs ~33MB)
