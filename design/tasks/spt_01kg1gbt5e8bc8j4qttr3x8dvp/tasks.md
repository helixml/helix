...
- [ ] Add `CreateSpecTaskAttachmentRequest` (multipart-driven on the handler side).
...
- [ ] Add `IsSpecTaskAttachmentPath` + `FilestoreSpecTaskUploadFile`/`FilestoreSpecTaskGet`/`FilestoreSpecTaskDelete` helpers in `api/pkg/controller/filestore.go`.
