...
- **GOTCHA: file picker — DO NOT use MUI's `Button` at all; use a plain `<label htmlFor>` paired with a sibling `<input id>`.** Took FOUR iterations to land on this. The story:
  1. `<Button onClick={() => inputRef.current?.click()}>` with `<input hidden>` — failed. Calling `.click()` on a `display: none` input breaks the user-gesture chain that browsers require to spawn the native picker dialog.
  2. `<Button component="label">` with a visually-hidden input *inside* — failed. MUI's `Button` adds `role="button"` and a click interceptor that swallows the click before the label/input mechanism fires.
  3. `<Button component="label" role={undefined} tabIndex={-1}>` with visually-hidden input inside — still failed in real-user clicks despite the chrome MCP test harness reporting the click chain worked. The ButtonBase + TouchRipple + ripple handlers stack still subtly broke something we couldn't isolate.
  4. **Final fix that actually works:** drop MUI Button entirely. Render an `<input id="…" type="file">` *outside* the button (`display: none`), and a `<Box component="label" htmlFor="…">` styled to look like a button. The browser's native `for`/`id` mechanism handles the click delegation with zero JS in the click path, which is the most rock-solid pattern across browsers.
  **Future file-upload UI in this codebase MUST follow pattern 4.** Anything with `<Button component="label">` will silently fail to open the picker for real users even if mcp test harnesses say it works. See `NewSpecTaskForm.tsx` line ~708 and `TaskAttachmentsPanel.tsx` line ~115 for the working pattern.
