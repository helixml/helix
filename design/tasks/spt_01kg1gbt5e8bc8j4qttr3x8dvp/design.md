...
- **GOTCHA: file picker button must use `<Button component="label">`, not `ref.click()`.** First implementation used `<Button onClick={() => inputRef.current?.click()}>` with a hidden `<input type="file">`. The mcp test harness reported the chain worked (button click → input click event fired), but real-user clicks in the browser did NOT open the native file picker. Cause: calling `.click()` programmatically on a `display: none` input (which the MUI `hidden` prop applies) breaks the user-gesture chain that browsers require to spawn the native picker dialog. **Fix:** use `<Button component="label">…<input type="file" /></Button>` with the input "visually hidden" (clip-path, 1×1 px, position absolute) rather than `display: none`. The native `<label>`→`<input>` relationship does the work without any JS, preserving the user gesture. Applied to both `NewSpecTaskForm` and `TaskAttachmentsPanel`. Future file-upload UI in this codebase should follow the same pattern.
