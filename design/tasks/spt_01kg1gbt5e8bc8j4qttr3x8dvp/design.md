...
- **GOTCHA: file picker button needs `Button component="label" role={undefined} tabIndex={-1}` + visually-hidden input.** Took two iterations to get right:
  1. First tried `<Button onClick={() => inputRef.current?.click()}>` with `<input hidden>`. The mcp test harness reported the chain worked (button click → input click event fired), but real-user clicks in the browser did NOT open the native file picker. Cause: calling `.click()` programmatically on a `display: none` input breaks the user-gesture chain that browsers require to spawn the native picker.
  2. Then tried `<Button component="label">` with a visually-hidden input inside. Still didn't open the dialog. Cause: MUI's `Button` adds `role="button"` and a click interceptor that swallows the click before the native `<label>`→`<input>` relationship can fire.
  3. **Final fix:** `<Button component="label" role={undefined} tabIndex={-1}>` with `<input type="file" />` inside, styled via `position: absolute; width: 1px; height: 1px; clip-path: inset(50%);` etc. (NOT `display: none`, NOT `hidden`). The `role={undefined}` disables MUI's button wrapping so the label semantics survive; the visually-hidden CSS keeps the input clickable to the browser without being visible to users. Applied to both `NewSpecTaskForm` and `TaskAttachmentsPanel`. **Future file-upload UI in this codebase MUST follow this exact pattern** — anything less subtle silently fails.
