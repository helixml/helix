...
- [ ] In `frontend/src/contexts/account.tsx`, extend the `orgNavigate` fallback chain to also try `organizationToolsRef.current.organizations?.[0]?.name`, and add a final guard that redirects to `/orgs` (with a `console.warn`) instead of calling `router.navigate` with `org_id: undefined`.
