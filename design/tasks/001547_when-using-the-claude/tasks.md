...
- [~] Replace `generateAgentServerConfig` in `main.go`: instead of returning `agent_servers.claude` config, write `/etc/claude-code/managed-settings.json` with `env` (including `ANTHROPIC_BASE_URL`) and `permissions.defaultMode`
