...
- [~] In `Dockerfile.sway-helix`, change `"RestoreOnStartup": 5` → `1` in the Chrome policy JSON (line 707) and add `"RestoreOnStartup":1` to the arm64 Chromium policy JSON (line 687 — currently has no RestoreOnStartup field).
