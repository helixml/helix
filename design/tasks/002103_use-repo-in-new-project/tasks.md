...
- [x] Test end-to-end in the inner Helix: from a repo page in an org, clicked "Use in Another Project" → repo (`testproj`) is now preselected in the dropdown, the dropdown is interactive (opens), and the mode tiles are unlocked (switching to "New Helix repo" works). Screenshots saved in `screenshots/`.
- [x] Regression note: the dialog still seeds the preselected repo + `select` mode via the existing useEffect; the Projects-page flow is unaffected (it passes no `preselectedRepoId`). Personal (no-org) workspace falls back to `{ ownerId: account.user?.id }`, mirroring Projects.tsx.
