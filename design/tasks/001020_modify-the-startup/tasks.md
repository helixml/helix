...
- [x] Modify `helix-3/desktop/shared/helix-workspace-setup.sh` to resolve symlinks when building Zed folders list
  - Use `readlink -f` on repo paths so Zed shows canonical names (e.g., `zed` instead of `zed-1`)
- [x] Test startup script handles already-correct naming
- [x] Test startup script handles container restart (symlinks allow helix-workspace-setup.sh to find repos)
- [x] Test Zed sidebar shows canonical names after symlink resolution