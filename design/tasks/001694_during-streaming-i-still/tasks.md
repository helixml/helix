...
- [x] In `handleMessageAdded` (`websocket_external_agent_sync.go`), track the current max entry index per interaction in `streamingContext`
- [x] When receiving a `message_added` event with `entry_type=tool_call` and a **new** entry index (> current max), force-flush by calling `publishEntryPatchesToFrontend` immediately (bypassing the 50ms `publishInterval` throttle)
- [x] After the force-flush, reset `sctx.lastPublish` so the throttle window restarts cleanly
- [x] Add a debug log: `"📤 [FLUSH] Force-published patches before tool_call entry N"`
