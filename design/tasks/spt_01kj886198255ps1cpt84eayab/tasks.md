...
- [ ] In `git_repository_handlers.go`: in `createGitRepositoryPullRequest`, check if repo is GitHub/GitLab; list user's connections; if no matching connection, find provider and call `s.oauthManager.StartOAuthFlow(...)`, return 401 with `{oauth_required, auth_url}`; if connection found, pass `user.ID` to `CreatePullRequest`
- [ ] In `spec_task_workflow_handlers.go`: check user OAuth connection upfront in `approveImplementation`; return 400 if missing; add `userID` param to `ensurePullRequestForTask`; pass `user.ID` through
