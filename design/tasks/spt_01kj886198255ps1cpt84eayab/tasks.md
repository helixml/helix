...
- [x] In `spec_task_workflow_handlers.go`: check user OAuth connection upfront in `approveImplementation`; return 400 if missing; add `userID` param to `ensurePullRequestForRepo` and `ensurePullRequestsForAllRepos`; pass `user.ID` as `approverUserID` through from `approveImplementation`
- [x] In `spec_task_orchestrator.go`: update `EnsurePRsFunc` type to include `userID string`; pass `""` from background polling (no user context)
