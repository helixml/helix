...
**US-2: Comment while no session exists**
> As a reviewer who has never started a planning session, when I submit a comment on a spec, I want a new planning session to be started automatically, so I don't have to navigate away to start one.

**US-3: Comment while desktop is already running**
...
- **AC-2:** When the user submits a comment and no planning session exists (`task.planning_session_id` is null/empty), the system automatically calls the start-planning API (`POST /api/v1/spec-tasks/{taskId}/start-planning`).
- **AC-3:** The comment is submitted to the database queue regardless of desktop state (the queue persists; the agent processes it once connected).
- **AC-4:** The UI shows feedback to the user that the desktop is being started (e.g. a transient message or status indicator).
- **AC-5:** If starting the desktop fails, the comment is still submitted and the user is shown an error message about the desktop start failure, but the comment is not lost.
- **AC-6:** If the desktop is already running, behavior is unchanged.
