...
- [x] In `frontend/src/components/tasks/SpecTaskDetailContent.tsx` (~line 464): remove `task.status !== "done"` and `task.status !== "pull_request"` from the `canMoveToBacklog` condition
- [x] In `frontend/src/components/tasks/TaskCard.tsx` (~line 610): remove `task.phase !== "completed"`, `task.phase !== "pull_request"`, `task.status !== "done"`, and `task.status !== "pull_request"` from the `canMoveToBacklog` condition
- [x] Verify `yarn build` passes with no type errors
