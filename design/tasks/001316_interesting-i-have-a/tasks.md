...
- [x] Add `detectExternalPRActivity()` function in `spec_task_orchestrator.go` that:
...
- [x] Call `detectExternalPRActivity()` from `prPollLoop()` after `pollPullRequests()`
- [x] Add rate limiting: process max 10 tasks per poll cycle
- [x] Add logging for PR detection and merged branch detection events
