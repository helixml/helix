...
- [x] **Creation half also proven e2e** (on an idle queue): clicked "Restart agent session" again → a **new** session `ses_…xjkw` (different id, status `running`) was minted, a **new** desktop container `ubuntu-external-…xjkw` spun up, the runtime pointer updated to the new session, and the **chat window rebound to the new session** showing its fresh transcript — with the trigger text *"An operator manually woke you up from the worker UI"* (distinct from the original *"You have just been hired… first activation"*), confirming a genuinely new session/thread. Screenshot: `screenshots/02-fresh-session-shown.png`.
  - Note: to get an idle queue I had to restart the API to clear a *stuck* first-activation (see finding below) — this inner Helix's desktop image can't register `claude-acp` ("Custom agent server `claude-acp` is not registered"), so turns never complete on their own. In a healthy env a settled bot's queue is already idle, so a single restart click does the whole flow.
