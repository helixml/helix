...
- [x] **Browser E2E on `localhost:8080`** (see `screenshots/`). Note: helix-org is off by default in the dev stack — enabled it via `HELIX_ORG_ENABLED=true` + granted the `helix-org` alpha feature to the test user (both are environment/account setup, not code):
  ...
  - [x] **Full end-to-end activation verified.** Created a real spec task in the watched project (projone) via `POST /spec-tasks/from-prompt` + `start-planning`. Its lifecycle emitted real attention events, which the publisher published onto the single `s-helix-events` topic with the generic envelope (verified via `GET /topics/s-helix-events/messages`):
    - `subject="Specs ready for review"`, `extra={domain:spectask, event_type:specs_pushed, project_id:…projone, spec_task_id:spt_…, project_name:projone}`, `thread_id=spt_…`, `message_id=atev_…`
    - `subject="Agent finished working"`, `event_type=agent_interaction_completed`
  - [x] **pm-bot (subscribed to `s-helix-events`) was activated by the event and responded.** Its transcript shows `=== activation: event e-26c6656e… on s-helix-events ===`, then it called `list_topics` / `list_topic_events` on `s-helix-events` and its LLM produced a correct summary: *"I've received a **specs_pushed** event for the project **projone** … Project ID prj_…, Spec task 'Add a README.md…'"*, then `=== exit: ok ===`. Screenshot `04-pm-bot-activated-by-event.png`. This proves the complete path: spec-task lifecycle → AttentionEvent → publisher → single `s-helix-events` topic → dispatch → subscribed bot activated + reacts. (The credential fix resolved the earlier `/v1/responses` issue for fresh event-driven activations.)
