...
- [~] Move `vm.ForceStop()` to the FIRST line of `shutdown()` — before tray.Stop() which can deadlock on main thread
- [~] Add SIGTERM/SIGINT handler in main.go as belt-and-suspenders for cases where shutdown() isn't reached
