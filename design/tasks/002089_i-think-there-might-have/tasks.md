...
- [ ] In `api/pkg/org/infrastructure/runtime/helix/project.go`, add `a.republishWorkerFiles(ctx, workerID, state.RepoID, roleContent, worker.IdentityContent())` inside the fast-path `else` block, immediately before `return state.ProjectID, state.AgentAppID, state.RepoID, nil`.
