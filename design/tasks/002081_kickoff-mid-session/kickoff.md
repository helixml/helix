...
> **Purpose of this document:** Original prompt for this spec task. Supersedes task [001806](../001806_high-leverage-for-us-to/) (`high-leverage-for-us-to`). Explains *why* we are restarting that work with a different approach and gives this task enough context to design and implement cleanly.

> **Reality-check note (2026-06-09):** Before starting, we audited the helix repo to confirm what "the earlier implementation" actually shipped. **Nothing.** Across all of `helix` git history, every search for `serializeTranscript`, `maybePrependTranscript`, `switchAgentSession`, `AgentThreadHistory`, `drainingThreadIDs`, `fork_seed` returns only `helix-specs` markdown commits — no `.go` file has ever defined these symbols. `feature/001806-high-leverage-for-us-to` is byte-identical to `main`. The "what survives from earlier work" table below is therefore **aspirational, not historical** — treat every listed piece as something to build from scratch on top of `main`, not something to preserve. Phase 0 (branch hygiene) collapses to "switch to the feature branch and start committing"; Phase 5 (remove old switch-agent path) is a no-op.
...
> See the reality-check note at the top — in practice, every "Keep" row needs to be built from scratch (the named symbols don't exist), and every "Remove" row is a no-op (nothing to delete).
...
The detailed plan lives in `design.md` / `tasks.md` next to this file. Top-level phases:

1. **Branch hygiene** — switch to `feature/001806-high-leverage-for-us-to` (already fast-forwarded to main; nothing to revert).
2. **Data model** — add lineage + pause fields to `SessionMetadata`.
...
4. **Backend: seed injection** — `maybePrependTranscript` with single trigger condition (read transcript from the `fork_seed.ResponseMessage` field).
...
6. **Backend: remove old switch-agent path** — no-op (nothing exists to remove).
7. **Frontend: chat panel** — add a new `AgentDropdown` instance in the chat panel header; `handleFork` calls `POST /sessions/{id}/fork` and navigates to the new session id.
8. **Frontend: lineage + pause UI** — `PausedBanner` (shows on paused sessions with link to child); `ForkBadge` (on children, links back to parent); `fork_seed` divider with expandable transcript disclosure.
...
Close it as superseded by this task (002081). The detailed design/tasks files written under `design/tasks/001806_high-leverage-for-us-to/` are preserved as a record of the architectural pivot — they explain *why* fork-and-pause was chosen over in-place mutation, with the validation matrix from the old approach showing what was successfully proved end-to-end (on a never-pushed local branch) before deciding it was the wrong shape.
