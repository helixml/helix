...
- [x] Verify in inner Helix (light + dark mode) that Review Spec, View Pull Request, and Create Task buttons all share the same colour, and the PR-state badge still shows merged/closed correctly — confirmed via screenshots (Review Spec teal matches NEW TASK; merged-PR button now teal/cyan not green, "merged" badge still shown)
