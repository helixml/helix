...
- [x] Add `pinProject` handler in `api/pkg/server/project_handlers.go` — `POST /api/v1/projects/{id}/pin` (append project ID to user's pinned list)
- [x] Add `unpinProject` handler in `api/pkg/server/project_handlers.go` — `DELETE /api/v1/projects/{id}/pin` (remove project ID from user's pinned list)
- [x] Register the two new routes in the server router
