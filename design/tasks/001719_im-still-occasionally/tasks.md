...
- [x] In `handleTextSelection` (~line 822), inside `processSelection`, call `removeHighlight()` only after confirming a valid new selection exists (not on empty clicks), then call `applyHighlight()` directly with the new range
