...
- The Pierre editor cache key remains stable across saves and changes only when the editor deliberately loads a different external revision. A save updates the confirmed baseline without remounting the editable document, and edits typed while a save is pending remain unsaved instead of being replaced by the response.
- Workspace-file saves refresh the review and file-tree metadata, but not the open file query. The PUT response is authoritative for the open editor; immediately refetching that file created unnecessary content churn.
- Inline comments use a compact neutral review card with line-range context. Opening a draft clears the transient blue line selection instead of leaving the selected block highlighted behind the composer.
- Sent file comments keep their structured `review_comment` transport envelope for the agent, while chat history parses that envelope into a readable file-comment card. The user sees the file path, line range, comment, and referenced code; transport attributes are never shown. Chat-turn previews use the same parsed representation.
