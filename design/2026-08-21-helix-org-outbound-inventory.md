...
| Webhook Topic `outbound_url` | External action, legacy compatibility | `get_secret` for optional auth, then `curl` to the non-secret URL held in Role/configuration | Native actions use the HTTP response as the receipt. Legacy Topic delivery remains asynchronous and returns no receipt | Publishing compatibility adapter and webhook transport tests |
...
| Topic REST/MCP publish and Processor output publish | Temporary compatibility | `publishing.LegacyDelivery` is the only provider-delivery decorator through PR 4 | Preserves legacy behavior: only events with a non-empty Worker source may deliver; inbound and Processor-output events never do | Publishing service and REST/MCP parity tests |
...
It is intentionally an application-layer seam: the webhook infrastructure
adapter depends on its narrow interface because this compatibility workflow is
application policy, not a permanent event-source domain capability.

Loop suppression has two explicit guards. Ingress calls `PublishInbound`, whose
context marker propagates through Processor traversal, and legacy delivery also
requires the resulting event to have a non-empty Worker source. Losing the
context marker must therefore not cause an inbound or Processor-output event to
echo externally. Tests pin both predicates.
