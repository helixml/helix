...
5. **Convince the customer's network team to open a UDP port** — And the security team. And probably the CTO. May be possible with sufficient amounts of whisky, fine wine, and steak dinners. Also requires implementing UDP port routing into their enterprise Kubernetes deployment, which only supports ingress right now. So, more whisky.
