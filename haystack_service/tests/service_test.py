...
from app.vectorchord.components import VectorchordEmbeddingRetriever, VectorchordBM25Retriever

# Path to test sample file
SAMPLE_PDF_PATH = os.path.join(os.path.dirname(__file__), "..", "test_files", "sample.pdf")

# Mark the asyncio tests specifically instead of the whole module
# This avoids warnings for non-async test functions
...
class TestHaystackService:
    ...
    def mock_converter(self):
        ...
        mock.run.return_value = {
            "documents": [
                Document(id="doc1", content="test content 1", meta={"source": "test.pdf"}),
                Document(id="doc2", content="test content 2", meta={"source": "test.pdf"})
            ]
        }
        return mock
    ...
    @pytest.fixture
    def service_with_mocks(self, mock_document_store, mock_converter, mock_vector_retriever, mock_bm25_retriever):
        """Create a service instance with mocked components"""
        with patch("app.service.VectorchordDocumentStore", return_value=mock_document_store):
            with patch("app.service.LocalUnstructuredConverter", return_value=mock_converter):
                with patch("app.service.Pipeline"):
                    with patch("app.service.VectorchordEmbeddingRetriever", return_value=mock_vector_retriever):
                        with patch("app.service.VectorchordBM25Retriever", return_value=mock_bm25_retriever):
                            with patch("app.service.DocumentJoiner"):
                                ...
                                with patch.object(HaystackService, '_init_indexing_pipeline'):
                                    with patch.object(HaystackService, '_init_query_pipeline'):
                                        ...
                                        service.bm25_document_store = mock_document_store
    ...
    async def test_extract_text(self, service_with_mocks):
        ...
        service_with_mocks.converter.run.assert_called_once_with(paths=["dummy_path.pdf"])
    ...
    async def test_query(self, service_with_mocks):
        ...
        docs = [
            Document(id="doc1", content="content1", meta={"source": "test1.pdf"}, score=0.95),
            Document(id="doc2", content="content2", meta={"source": "test2.pdf"}, score=0.85)
        ]
        ...
        # Perform the query
        results = await service_with_mocks.query("test query", filters={"key": "value"}, top_k=2)
    ...
    async def test_query_sanitize_null_bytes(self, service_with_mocks):
        ...
        service_with_mocks.query_pipeline.run.return_value = {"document_joiner": {"documents": []}}
    ...
    async def test_delete(self, service_with_mocks):
        ...
        service_with_mocks.document_store.filter_documents.assert_called_once_with(filters={"source": "test.pdf"})
        service_with_mocks.document_store.delete_documents.assert_called_once_with(["doc1", "doc2"])
    ...
    def test_analyze_scores_empty_input(self, service_with_mocks):
        ...
        assert stats == {} or (
            ...
            stats.get("vector", {}).get("count", 0) == 0 and
            stats.get("bm25", {}).get("count", 0) == 0
        )
