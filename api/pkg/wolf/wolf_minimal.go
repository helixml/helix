...
// In apps mode, Wolf uses direct video capture - don't specify custom pipelines
func NewMinimalDockerApp(id, title, name, image string, env, mounts []string, baseCreateJSON string, displayWidth, displayHeight, displayFPS int) *MinimalWolfApp {
	return &MinimalWolfApp{
		...
		// Apps mode: Wolf generates direct capture pipelines automatically
		// Do NOT specify custom pipelines - interpipe is for lobbies mode only
		VideoProducerBufferCaps: nil,  // Let Wolf use defaults for direct capture
		H264GstPipeline:         nil,
		HEVCGstPipeline:         nil,
		AV1GstPipeline:          nil,
		OpusGstPipeline:         nil,
