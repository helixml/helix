...
// Sandbox-absorbs-runner pivot: GetDashboardData /
// getGGUFBasedMemoryEstimateForDashboard / GetSchedulerHeartbeats /
// DeleteSlotFromScheduler all gone with the scheduler. The admin
// dashboard reads SandboxInstance + RunnerProfile directly from the
// store now; per-sandbox health flows via the existing sandbox heartbeat.
