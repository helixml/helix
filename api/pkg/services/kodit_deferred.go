...
// DeferredKoditService wraps a KoditServicer with an atomically-swappable
// inner. Used because kodit.New() probes the configured embedding endpoint,
// which may not be up when InitKodit is called during server startup — so the
// real kodit client is initialised asynchronously and swapped in once ready.
...
// disabled service. Callers must call Set() once kodit.New() succeeds.
...
// Set atomically swaps the inner service.
