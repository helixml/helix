...
func (m *DesignDocsWorktreeManager) InitializeTaskDirectory(worktreePath string, taskID string, taskName string) (string, error) {
	...
	// Create requirements.md template (Kiro's EARS format)
...
<!-- EARS format: Event, Action, Response, Success criteria -->
<!-- Example:
...
---
*Generated by Helix Planning Agent - Following Kiro.dev approach*
`, taskID, time.Now().Format("2006-01-02"))
	...
	// Create design.md template (Kiro's technical design format)
...
<!-- Key interaction flows and sequences -->

## Data Model

<!-- Database schema changes, API contracts, data structures -->
...
<!-- Technical decisions, tradeoffs, risks -->
...
`, taskID, time.Now().Format("2006-01-02"))
	...
	// Create tasks.md template (Kiro's discrete task breakdown)
	tasksTemplate := fmt.Sprintf(`# Implementation Tasks
...
<!-- Planning agent: Break down implementation into discrete tasks -->
...
---
*Discrete task tracking following Kiro.dev methodology*
...
// ParseTaskList parses the task list from progress.md
func (m *DesignDocsWorktreeManager) ParseTaskList(worktreePath string) ([]TaskItem, error) {
	progressPath := filepath.Join(worktreePath, "progress.md")
...
func (m *DesignDocsWorktreeManager) updateTaskStatus(worktreePath string, lineNumber int, statusChar string) error {
	...
	err = os.WriteFile(progressPath, []byte(newContent), 0644)
	if err != nil {
