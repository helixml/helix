...
// to implement independently: UpdateIdentity (per-Worker persona),
// UpdateRole (rewrite the content of the Role a Worker holds), and the
// reporting-line edges (AddParent / RemoveParent).
//
// Hire and Fire — the create/destroy halves of the worker lifecycle —
// live together in the lifecycle service, not here. This package is the
// pure row-mutation surface for an existing Worker.
//
