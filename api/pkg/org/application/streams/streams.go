...
// Stream use cases — Create, Update, Delete. It is the single home for
// the stream-mutation logic that the MCP `create_stream` tool and the
// REST stream handlers used to each implement independently. Both
// adapters now parse their protocol and delegate here, so the
// invariants (auto-id, transport defaulting, the update read-modify-
// write merge, org-scoping) cannot drift between callers.
//
