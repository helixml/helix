// Package topology is the application-layer reconciler that converges the
// persisted Streams/Subscriptions onto the channels the reporting graph
// requires. The pure derivation — "what Streams and Subscriptions does
// this graph require?" — lives in domain/channels; this package loads the
// graph from the store, calls channels.Required, diffs the required set
// against what's persisted, and applies create/subscribe/unsubscribe/
// delete idempotently.
//
...
// *what changed* by calling Reconcile; the reconciler decides the stream
// consequences. Event-specific deltas drift; a declarative diff can't.
package topology
