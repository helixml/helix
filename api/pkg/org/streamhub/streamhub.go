...
// that preserves the typed stream.ID API the helix-org call sites used
...
// publish to.
func topicFor(sid stream.ID) string {
...
// behaviour.
func (h *Hub) Subscribe(streamIDs []stream.ID) chan struct{} {
...
func (h *Hub) Unsubscribe(streamIDs []stream.ID, ch chan struct{}) {
	if len(streamIDs) == 0 {
		return
	}
	h.mu.Lock()
...
func (h *Hub) Notify(streamID stream.ID) {
