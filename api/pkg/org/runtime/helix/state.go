...
func LoadState(ctx context.Context, st *store.Store, orgID string, workerID worker.ID) (WorkerState, error) {
...
func SaveHiringUser(ctx context.Context, st *store.Store, orgID string, workerID worker.ID, userID string) error {
	if userID == "" {
		return nil
	}
	if st == nil || st.WorkerRuntimeState == nil {
...
func SaveProject(ctx context.Context, st *store.Store, orgID string, workerID worker.ID, projectID, agentAppID, repoID string) error {
	if st == nil || st.WorkerRuntimeState == nil {
		return errors.New("helix state: store is nil")
	}
...
func SaveSession(ctx context.Context, st *store.Store, orgID string, workerID worker.ID, sessionID string) error {
	if st == nil || st.WorkerRuntimeState == nil {
		return errors.New("helix state: store is nil")
	}
...
func ClearProject(ctx context.Context, st *store.Store, orgID string, workerID worker.ID) error {
	if st == nil || st.WorkerRuntimeState == nil {
		return errors.New("helix state: store is nil")
	}
	return st.WorkerRuntimeState.SetMany(ctx, orgID, workerID, Backend, map[string]string{
