...
func (a *apiHandler) createRole(w http.ResponseWriter, r *http.Request) {
	...
	rl, err := role.New(role.ID(strings.TrimSpace(req.ID)), req.Content, tools, streams, a.deps.Now().UTC(), orgID)
	if err != nil {
		writeError(w, http.StatusBadRequest, err)
		return
	}
...
func (a *apiHandler) createPosition(w http.ResponseWriter, r *http.Request) {
	...
	pos, err := domain.NewPosition(position.ID(strings.TrimSpace(req.ID)), role.ID(strings.TrimSpace(req.RoleID)), parent, orgID)
	if err != nil {
		writeError(w, http.StatusBadRequest, err)
		return
	}
...
func (a *apiHandler) updatePosition(w http.ResponseWriter, r *http.Request) {
	...
	if req.RoleID != nil {
		existing.RoleID = role.ID(strings.TrimSpace(*req.RoleID))
	}
	if req.ParentID != nil {
		if p := strings.TrimSpace(*req.ParentID); p == "" {
...
func toStreamIDs(in []string) []stream.ID {
	if len(in) == 0 {
		return nil
	}
	out := make([]stream.ID, 0, len(in))
	for _, s := range in {
		if t := strings.TrimSpace(s); t != "" {
			out = append(out, stream.ID(t))
		}
	}
	return out
}
