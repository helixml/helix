...
// (domain.WorkerKind -> orgchart.WorkerKind, domain.WorkerKindHuman ->
// orgchart.WorkerKindHuman, etc.) per the B1 stutter-removal precedent. The
...
func TestKindValues_ReturnsHumanThenAIInOrder(t *testing.T) { // W1
	t.Parallel()
	got := orgchart.WorkerKindValues()
	if len(got) != 2 {
		t.Fatalf("WorkerKindValues() len = %d, want 2 (%v)", len(got), got)
	}
	if got[0] != orgchart.WorkerKindHuman {
		t.Fatalf("WorkerKindValues()[0] = %q, want %q", got[0], orgchart.WorkerKindHuman)
	}
	if got[1] != orgchart.WorkerKindAI {
		t.Fatalf("WorkerKindValues()[1] = %q, want %q", got[1], orgchart.WorkerKindAI)
	}
}

func TestValidate_AcceptsHuman(t *testing.T) { // W2
	t.Parallel()
	if err := orgchart.WorkerKindHuman.Validate(); err != nil {
...
func TestValidate_AcceptsAI(t *testing.T) { // W3
	t.Parallel()
	if err := orgchart.WorkerKindAI.Validate(); err != nil {
...
func TestValidate_RejectsEmpty(t *testing.T) { // W4
	t.Parallel()
	err := orgchart.WorkerKind("").Validate()
	if err == nil {
...
func TestValidate_RejectsBogus(t *testing.T) { // W5
	t.Parallel()
	err := orgchart.WorkerKind("bogus").Validate()
	if err == nil {
...
func TestValidate_UnknownErrorListsOffendingValueAndValidOptions(t *testing.T) { // W6
	t.Parallel()
	err := orgchart.WorkerKind("claude").Validate()
	if err == nil {
