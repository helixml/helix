...
func TestNewBotRejectsTraversalID(t *testing.T) {
	t.Parallel()
	now := time.Now()
	if _, err := NewNode("../../etc/cron.d/x", "content", nil, now, "org-test"); err == nil {
		t.Fatal("NewNode with traversal id: want error")
	}
	if _, err := NewNode("a/b", "content", nil, now, "org-test"); err == nil {
		t.Fatal("NewNode with separator id: want error")
	}
}
