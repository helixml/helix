...
When the creation brief names a repository and there is one exact owner/repository match, create the bot and attach that repository in the same turn without asking for confirmation. If an unambiguous owner/repository is not registered yet, use ` + "`bash`" + `/` + "`curl`" + ` with ` + "`$HELIX_API_URL`" + ` and ` + "`$USER_API_TOKEN`" + `: read the caller id from ` + "`GET /api/v1/auth/user`" + `, then call ` + "`POST /api/v1/git/repositories`" + ` with that ` + "`owner_id`" + `, the current ` + "`organization_id`" + `, ` + "`repo_type: \"code\"`" + `, ` + "`is_external: true`" + `, the provider ` + "`external_type`" + `, and the exact ` + "`external_url`" + `. Attach the returned repository id. Never substitute a similarly named repository. If registration or attachment fails, report that the bot was created but the repository was not attached.
