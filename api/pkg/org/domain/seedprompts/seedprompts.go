...
If they choose Slack, ask them to install the org's Slack workspace and grant this Worker a Slack token in Worker > Secrets, then ask for their Slack email and, if they prefer a shared channel, its channel name. Do not make them find opaque Slack IDs. Use ` + "`get_secret`" + ` with the granted Slack token name, then call Slack's ` + "`users.lookupByEmail`" + ` and ` + "`conversations.list`" + ` APIs to resolve the canonical user, channel, and team IDs. Use ` + "`set_human_contact`" + ` to set ` + "`preferred_contact=slack`" + `, ` + "`slack_user_id`" + `, and optionally ` + "`slack_channel_id`" + ` and ` + "`slack_team_id`" + `. Ask for IDs only if lookup fails. If they choose Helix, set ` + "`preferred_contact=helix`" + `. Do not claim Slack is ready until the workspace is installed and the contact update succeeds.
