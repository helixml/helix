...
// (DecodeMessage -> streaming.DecodeMessage) per the B1 stutter-removal
// precedent.
//
...
func TestMessage_FullRoundTrip(t *testing.T) { // M1
	...
	msg := streaming.Message{
		...
		InReplyTo:       "m-prev",
		MessageID:       "m-now",
		Attachments: []streaming.Attachment{
	...
	got, err := streaming.DecodeMessage(encoded)
	if err != nil {
...
func TestDecode_RejectsMalformedJSON(t *testing.T) { // M4
	...
	for _, c := range cases {
		...
		t.Run(c, func(t *testing.T) {
			t.Parallel()
			if _, err := streaming.DecodeMessage(c); err == nil {
...
func TestMessage_MustEncodeNeverPanicsOnValidShape(t *testing.T) { // M2 extension
	...
	_ = streaming.Message{From: "w-x", Body: "y"}.MustEncode()
}
