...
func TestPrincipalConstructorsPinKindAndID(t *testing.T) {
	...
	cases := []struct {
		name      string
		got       streaming.Principal
		wantKind  streaming.PrincipalKind
		wantID    string
		wantValid bool
	}{
		{"worker", streaming.NewPrincipalWorker("w-alice"), streaming.PrincipalKindWorker, "w-alice", true},
		{"transport", streaming.NewPrincipalTransport("alice@example.com"), streaming.PrincipalKindTransport, "alice@example.com", true},
		{"human", streaming.NewPrincipalHuman("ops-1"), streaming.PrincipalKindHuman, "ops-1", true},
	}
...
func TestPrincipalValidateRejectsPartial(t *testing.T) {
	t.Parallel()
	cases := []streaming.Principal{
		{Kind: streaming.PrincipalKindWorker},             // ID missing
		{ID: "w-alice"},                                   // Kind missing
		{Kind: streaming.PrincipalKind("admin"), ID: "x"}, // unknown Kind
	}
	for i, p := range cases {
		p := p
...
func TestPrincipalJSONRoundTrip(t *testing.T) {
	t.Parallel()
	want := streaming.NewPrincipalWorker("w-alice")
	...
	encodedZero, err := json.Marshal(streaming.Principal{})
	if err != nil {
