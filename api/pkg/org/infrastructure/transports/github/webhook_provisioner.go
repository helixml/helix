...
func (p *WebhookProvisioner) Install(ctx context.Context, orgID string, stream streaming.Stream) (streaming.InstallResult, error) {
	publicURL := p.resolvePublicURL(ctx, orgID)
	if publicURL == "" {
		return streaming.InstallResult{}, fail(streaming.FailPrecondition, "no public URL configured for helix. Set `streams.public_url` on the helix-org Settings page (or SERVER_URL in helix's .env), then re-install the webhook.")
	}
	if u, err := url.Parse(publicURL); err == nil {
		host := strings.ToLower(u.Hostname())
		if host == "localhost" || host == "127.0.0.1" || host == "0.0.0.0" {
			return streaming.InstallResult{}, fail(streaming.FailPrecondition, "public URL %q is a loopback address — GitHub refuses to install webhooks pointed at unreachable hosts. Set `streams.public_url` on the helix-org Settings page to a publicly reachable hostname (cloudflared / ngrok / reverse proxy), or update SERVER_URL in helix's .env and restart the api container", publicURL)
		}
	}
	...
	if cfg.Repo == "" {
		return streaming.InstallResult{}, fail(streaming.FailBadRequest, "stream's github config has no repo set; edit the stream first")
	}
	if len(cfg.Events) == 0 {
		cfg.Events = []string{"*"}
	}
...
func (p *WebhookProvisioner) Status(ctx context.Context, orgID string, stream streaming.Stream) (streaming.InboundState, error) {
	cfg, err := stream.Transport.GitHubConfig()
	if err != nil {
