...
func (r *reportingLinesRepo) Remove(ctx context.Context, orgID string, reportID, managerID orgchart.NodeID) error {
...
func (r *reportingLinesRepo) List(ctx context.Context, orgID string) ([]orgchart.ReportingLine, error) {
	...
	for _, row := range rows {
		out = append(out, orgchart.ReportingLine{
			OrgID:     row.OrgID,
			ManagerID: orgchart.NodeID(row.ManagerID),
			ReportID:  orgchart.NodeID(row.ReportID),
		})
	}
	return out, nil
}

func (r *reportingLinesRepo) ListManagers(ctx context.Context, orgID string, reportID orgchart.NodeID) ([]orgchart.NodeID, error) {
	var rows []reportingLineRow
	if err := r.db.WithContext(ctx).
	...
	for _, row := range rows {
		out = append(out, orgchart.NodeID(row.ManagerID))
	}
	return out, nil
}

func (r *reportingLinesRepo) ListReports(ctx context.Context, orgID string, managerID orgchart.NodeID) ([]orgchart.NodeID, error) {
	var rows []reportingLineRow
	if err := r.db.WithContext(ctx).
	...
	out := make([]orgchart.NodeID, 0, len(rows))
	for _, row := range rows {
		out = append(out, orgchart.NodeID(row.ReportID))
	}
	return out, nil
}
