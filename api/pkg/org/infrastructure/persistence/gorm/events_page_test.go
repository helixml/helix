...
func TestEventsDeleteForStream(t *testing.T) {
	t.Parallel()
	s := newStore(t)
	ctx := context.Background()
	...
	for _, item := range []struct {
		...
		ev, _ := streaming.NewEvent(streaming.EventID(item.id), streaming.StreamID(item.topicID), "w-owner", "body", now, item.orgID)
		if err := s.Events.Append(ctx, ev); err != nil {
			t.Fatalf("Append %s: %v", item.id, err)
		}
	}

	if err := s.Events.DeleteForStream(ctx, "org-test", "s-a"); err != nil {
		t.Fatalf("DeleteForStream: %v", err)
	}
	if count, err := s.Events.CountForStream(ctx, "org-test", "s-a"); err != nil || count != 0 {
		t.Fatalf("cleared count = %d, %v; want 0, nil", count, err)
	}
	if count, _ := s.Events.CountForStream(ctx, "org-test", "s-b"); count != 1 {
		t.Fatalf("other topic count = %d, want 1", count)
	}
	if count, _ := s.Events.CountForStream(ctx, "org-other", "s-a"); count != 1 {
	...
	if count, _ := s.Events.CountForStream(ctx, "org-test", "s-a"); count != 1 {
