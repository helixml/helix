...
func (t *GetSecret) Description() string {
	return "Return the current value of one credential explicitly granted to this Worker. Call immediately before an authenticated operation and again after a 401/403. The value is sensitive: do not print it or place it in command-line arguments. Args: name. Backend source and resource IDs are intentionally not accepted. An error for an unknown name means the credential is not bound to this Worker — it does not mean the value is unreachable: project-scoped secrets are injected into the container environment, so check there before reporting a credential as unavailable."
}
