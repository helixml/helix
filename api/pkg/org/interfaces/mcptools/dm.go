...
// channel), not something any Worker can do implicitly to anyone.
//
// The Trigger ID is deterministic from the sorted pair
// (channels.DMTriggerID), so A→B and B→A land on the same channel and
// the back-and-forth stays ordered in one place. The managers / reports
...
func (t *DM) Description() string {
	return "Send a direct message (DM/PM/private message) to a single other Bot. " +
		...
		"exists. Sends the body and returns the triggerId — read_events on it " +
...
func (t *DM) Invoke(ctx context.Context, inv tool.Invocation) (json.RawMessage, error) {
	...
	// never sanctioned.
	channelID := channels.DMTriggerID(sender, recipient)
	if _, err := t.deps.Queries.GetTrigger(ctx, orgID, channelID); err != nil {
		if errors.Is(err, store.ErrNotFound) {
			return nil, fmt.Errorf(
				"no DM channel with %q: you can only DM bots you share a reporting line with — "+
					...
					"otherwise escalate to a manager or brief via your team chat",
				recipient, recipient)
		}
		return nil, fmt.Errorf("lookup dm channel %q: %w", channelID, err)
	}
	...
	// append → notify → route trio. dm must NOT reimplement it (that
	// is how the DM fan-out drifts from every other send path).
	event, err := t.deps.Publishing.SendToChannel(ctx, orgID, channelID, string(sender), msg)
	if err != nil {
		return nil, err
	}
