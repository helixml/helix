...
func TestPublishParity_RESTvsMCP(t *testing.T) {
	...
	args, _ := json.Marshal(map[string]any{"topicId": "s-1", "body": "hello world", "subject": "hi"})
	if _, err := publish.Invoke(ctx, tool.Invocation{Caller: ownerCaller(t), Args: args}); err != nil {
		t.Fatalf("MCP publish: %v", err)
	}

	restEvents, _ := restStore.Events.ListForTopic(ctx, "org-test", "s-1", 10)
	mcpEvents, _ := mcpStore.Events.ListForTopic(ctx, "org-test", "s-1", 10)
