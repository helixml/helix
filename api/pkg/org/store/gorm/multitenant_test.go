...
func TestWorkers_SameIDAcrossOrgs(t *testing.T) {
	...
	for _, org := range []string{orgA, orgB} {
		r, err := role.New("r-owner", "# Owner", nil, nil, time.Now().UTC(), org)
		if err != nil {
			t.Fatalf("role.New(%s): %v", org, err)
		}
		if err := s.Roles.Create(ctx, r); err != nil {
			t.Fatalf("Roles.Create(%s): %v", org, err)
		}
		pos, err := domain.NewPosition("p-root", "r-owner", nil, org)
		if err != nil {
		...
		w, err := domain.NewHumanWorker("w-owner", "p-root", "# Owner identity", org)
		if err != nil {
...
func mustSeedOwner(t *testing.T, s *store.Store, orgID string) {
	...
	r, err := role.New("r-owner", "# Owner", nil, nil, time.Now().UTC(), orgID)
	if err != nil {
	...
	pos, err := domain.NewPosition("p-root", "r-owner", nil, orgID)
	if err != nil {
	...
	w, err := domain.NewHumanWorker("w-owner", "p-root", "# Owner", orgID)
	if err != nil {
