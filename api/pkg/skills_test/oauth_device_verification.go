...
// GitHubDeviceVerificationHandler handles GitHub device verification using Gmail
type GitHubDeviceVerificationHandler struct {
	logger            zerolog.Logger
	gmailService      *gmail.Service
	browserAutomation *OAuthBrowserAutomation
}

// NewGitHubDeviceVerificationHandler creates a new GitHub device verification handler
func NewGitHubDeviceVerificationHandler(logger zerolog.Logger, gmailCredentialsBase64 string, browserAutomation *OAuthBrowserAutomation) (*GitHubDeviceVerificationHandler, error) {
	gmailService, err := setupGmailService(gmailCredentialsBase64)
	if err != nil {
		return nil, fmt.Errorf("failed to setup Gmail service: %w", err)
	}

	return &GitHubDeviceVerificationHandler{
		logger:            logger,
...
func setupGmailService(gmailCredentialsBase64 string) (*gmail.Service, error) {
	// Decode base64 credentials
	credentials, err := base64.StdEncoding.DecodeString(gmailCredentialsBase64)
	if err != nil {
		return nil, fmt.Errorf("failed to decode Gmail credentials: %w", err)
	}
	...
	if err != nil {
		return nil, fmt.Errorf("failed to parse Gmail credentials: %w", err)
	}
...
func (h *GitHubDeviceVerificationHandler) HandleDeviceVerification(page *rod.Page) error {
	h.logger.Info().Msg("Checking if GitHub device verification is required")
	...
	if !isDeviceVerificationPage {
		h.logger.Info().Str("url", currentURL).Msg("Not on device verification page, continuing normal flow")
		return nil
	}

	h.logger.Info().Str("url", currentURL).Msg("GitHub device verification page detected")
	h.browserAutomation.TakeScreenshot(page, "github_device_verification_detected")

	// Handle different device verification page types
	if strings.Contains(currentURL, "/sessions/verified-device") {
		return h.handleVerifiedDevicePage(page)
	}

	if strings.HasSuffix(currentURL, "/session") {
		return h.handleSessionPage(page)
	}

	if strings.Contains(currentURL, "/login/device") {
		return h.handleTraditionalDeviceVerification(page)
	}
...
func (h *GitHubDeviceVerificationHandler) handleVerifiedDevicePage(page *rod.Page) error {
	...
	if codeInput != nil {
		return h.handleDeviceVerificationInput(page, codeInput)
	}

	// If no code input found, look for continue buttons or other actions
	return h.handleContinueButton(page)
}

// handleSessionPage handles GitHub's new session verification flow
func (h *GitHubDeviceVerificationHandler) handleSessionPage(page *rod.Page) error {
	...
	if err == nil && deviceCodeInput != nil {
		...
		return h.handleDeviceVerificationInput(page, deviceCodeInput)
	}

	// Handle continue buttons or forms
	return h.handleContinueButton(page)
}

// handleTraditionalDeviceVerification handles the traditional device verification flow
func (h *GitHubDeviceVerificationHandler) handleTraditionalDeviceVerification(page *rod.Page) error {
	h.logger.Info().Msg("Handling traditional device verification")
	h.browserAutomation.TakeScreenshot(page, "device_verification_detected")
...
func (h *GitHubDeviceVerificationHandler) handleDeviceVerificationInput(page *rod.Page, codeInput *rod.Element) error {
	...
	h.browserAutomation.TakeScreenshot(page, "device_verification_code_entered")
	...
	h.browserAutomation.TakeScreenshot(page, "device_verification_submitted")
...
// handleContinueButton handles continue buttons on session pages
func (h *GitHubDeviceVerificationHandler) handleContinueButton(page *rod.Page) error {
	...
	if err == nil && continueButton != nil {
		...
		h.browserAutomation.TakeScreenshot(page, "session_page_continue_button_found")
	...
	if err == nil && len(forms) > 0 {
		...
		for i, form := range forms {
			...
			if err == nil && submitBtn != nil {
				...
				h.browserAutomation.TakeScreenshot(page, "session_page_form_submit_found")
	...
	if newURL != page.MustInfo().URL {
		h.logger.Info().Str("new_url", newURL).Msg("Session page redirected automatically")
	}
