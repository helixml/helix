...
// startExternalAgentRunnerWebSocketServer starts a WebSocket server to which external agent runners can connect
...
func (apiServer *HelixAPIServer) startExternalAgentRunnerWebSocketServer(r *mux.Router, path string) {
	r.HandleFunc(path, func(w http.ResponseWriter, r *http.Request) {
		...
		// Subscribe to external agent tasks
		externalAgentSub, err := apiServer.pubsub.StreamConsume(ctx, pubsub.ExternalAgentRunnerStream, pubsub.ExternalAgentQueue, func(msg *pubsub.Message) error {
			...
			err := wsConn.WriteJSON(&types.RunnerEventRequestEnvelope{
				...
				Type:      messageType,
				Payload:   msg.Data, // The actual payload (external agent request)
			})
			if err != nil {
		...
		if err != nil {
			log.Error().Msgf("Error subscribing to external agent queue: %s", err.Error())
			return
		}
		defer func() {
			if err := externalAgentSub.Unsubscribe(); err != nil {
				log.Err(err).Msg("failed to unsubscribe from external agent queue")
			}
		}()
