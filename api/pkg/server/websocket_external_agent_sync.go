...
func (apiServer *HelixAPIServer) handleMessageAdded(sessionID string, syncMsg *types.SyncMessage) error {
	...
	if role == "assistant" {
		...
		if targetInteraction != nil {
			...
			// Even without an interaction, we may have a design review comment waiting for a response
			// This happens when comments are sent via sendMessageToSpecTaskAgent which doesn't create interactions
			pendingComment, err := apiServer.Store.GetPendingCommentByPlanningSessionID(context.Background(), helixSessionID)
			if err == nil && pendingComment != nil {
				commentID := pendingComment.ID
				requestID := pendingComment.RequestID
				...
				log.Info().
					...
					Str("session_id", helixSessionID).
					Str("request_id", requestID).
					Msg("📝 [HELIX] Found pending comment for session (no interaction) - updating with streaming response")
				...
				go func(sessionID, commentID, requestID, responseContent string) {
					if err := apiServer.updateCommentWithStreamingResponse(context.Background(), requestID, responseContent); err != nil {
						log.Debug().
							...
							Str("session_id", sessionID).
							Str("request_id", requestID).
							Msg("Failed to update comment with streaming response")
						return
					}
					...
					log.Info().
						...
						Str("request_id", requestID).
						Int("response_length", len(responseContent)).
						Msg("✅ [HELIX] Updated comment with streaming agent response (no interaction path)")
				}(helixSessionID, commentID, requestID, content)

				// Reload session and publish update to frontend for real-time streaming
				reloadedSession, err := apiServer.Controller.Options.Store.GetSession(context.Background(), helixSessionID)
				if err != nil {
					log.Error().Err(err).Str("session_id", helixSessionID).Msg("Failed to reload session for comment streaming")
				} else {
					// Load all interactions so WebSocket event has data
					allInteractions, _, err := apiServer.Controller.Options.Store.ListInteractions(context.Background(), &types.ListInteractionsQuery{
						SessionID:    helixSessionID,
						GenerationID: reloadedSession.GenerationID,
						PerPage:      1000,
					...
					if err == nil {
						...
						// Create a synthetic interaction for the publish (frontend expects interaction data)
						syntheticInteraction := &types.Interaction{
						...
						log.Info().
							...
							Msg("📤 [HELIX] Publishing session update for comment streaming (no interaction path)")

						// Publish to frontend - pass requestID so commenter receives the update
						err = apiServer.publishSessionUpdateToFrontend(reloadedSession, syntheticInteraction, requestID)
						if err != nil {
							log.Error().Err(err).
								Str("session_id", helixSessionID).
								Str("request_id", requestID).
								Msg("Failed to publish session update to frontend for comment streaming")
						}
					}
				}
			}
