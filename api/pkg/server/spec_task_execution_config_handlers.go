...
func (s *HelixAPIServer) resolveSpecTaskExecutionConfig(ctx context.Context, task *types.SpecTask) (*types.SpecTaskExecutionConfig, error) {
	...
	if !config.AgentAvailable && task.AgentSessionID != "" {
...
func (s *HelixAPIServer) updateSpecTaskExecutionConfig(w http.ResponseWriter, r *http.Request) {
	...
	if req.SandboxResourceOverrides != nil {
		...
		if task.AgentSessionID != "" && s.externalAgentExecutor != nil && s.externalAgentExecutor.HasRunningContainer(ctx, task.AgentSessionID) {
			if err := s.externalAgentExecutor.UpdateDesktopResources(ctx, task.AgentSessionID, req.SandboxResourceOverrides); err != nil {
		...
		if err := s.Store.UpdateSpecTask(ctx, task); err != nil {
			if response.SandboxResourcesApplied {
				...
				if rollbackErr := s.externalAgentExecutor.UpdateDesktopResources(context.Background(), task.AgentSessionID, rollback); rollbackErr != nil {
