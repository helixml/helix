...
func (apiServer *HelixAPIServer) getZedConfig(_ http.ResponseWriter, req *http.Request) (*types.ZedConfigResponse, *system.HTTPError) {
	...
	// Get project MCPs if session has a project
	var projectMCPs []types.AssistantMCP
	if session.ProjectID != "" {
		project, err := apiServer.Store.GetProject(ctx, session.ProjectID)
		if err != nil {
			log.Error().Err(err).Str("project_id", session.ProjectID).Msg("Failed to get project for MCP config")
			return nil, system.NewHTTPError500("failed to get project for MCP config")
		}
	...
	zedConfig, err := external_agent.GenerateZedMCPConfig(app, session.Owner, sessionID, sandboxAPIURL, helixToken, koditEnabled, projectMCPs)
	if err != nil {
...
func (apiServer *HelixAPIServer) getMergedZedSettings(_ http.ResponseWriter, req *http.Request) (map[string]interface{}, *system.HTTPError) {
	...
	zedConfig, err := external_agent.GenerateZedMCPConfig(app, session.Owner, sessionID, helixAPIURL, helixToken, apiServer.Cfg.Kodit.Enabled, projectMCPs)
	if err != nil {
		log.Error().Err(err).Msg("Failed to generate Zed config")
		return nil, system.NewHTTPError500("failed to generate Zed config")
	}
