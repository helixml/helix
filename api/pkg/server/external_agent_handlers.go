...
func (apiServer *HelixAPIServer) createExternalAgent(res http.ResponseWriter, req *http.Request) {
	...
	// Auto-join the Wolf UI client to the lobby (Phase 3: Multi-tenancy)
	if response.WolfLobbyID != "" {
		err = apiServer.autoJoinWolfLobby(req.Context(), response.WolfLobbyID, response.WolfLobbyPIN)
		if err != nil {
			log.Warn().Err(err).
				Str("lobby_id", response.WolfLobbyID).
				Msg("Failed to auto-join Wolf UI to lobby - user will need to join manually")
			// Continue anyway - user can still join manually via Wolf UI
		} else {
			log.Info().
				Str("lobby_id", response.WolfLobbyID).
				Msg("✅ Auto-joined Wolf UI client to lobby")
		}
	}
...
// autoJoinWolfLobby automatically joins the Wolf UI client to the specified lobby
// This allows users to be auto-switched to their lobby without manual navigation
func (apiServer *HelixAPIServer) autoJoinWolfLobby(ctx context.Context, lobbyID string, lobbyPIN string) error {
	...
	log.Info().
		...
		Msg("Found Wolf UI app, querying sessions to find client")
	...
	log.Info().
		...
		Msg("Calling Wolf API to join lobby")
