...
// rewrite HelixAppID — AgentSessionID stays the same. Best-effort: failures
...
func (apiServer *HelixAPIServer) repointSpecTaskForSwitch(ctx context.Context, session *types.Session, targetAppID string) {
	...
	// Also cover any spec task that tracks this session via AgentSessionID
	// (defensive: keeps the spec-task page / other lookups consistent).
	tasks, err := apiServer.Store.ListSpecTasks(ctx, &types.SpecTaskFilters{AgentSessionID: session.ID})
	if err != nil {
		return
	}
