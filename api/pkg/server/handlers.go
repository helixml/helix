...
func (apiServer *LilysaasAPIServer) createSession(res http.ResponseWriter, req *http.Request) (*types.Session, error) {
	...
	// so far it's a chat with one message
	...
	s, err := apiServer.Store.CreateSession(reqContext.Ctx, session)
	if err != nil {
		return nil, err
	}

	// use the returned session in case any autogenerated-by-database values (e.g. created at time) changed
	session = *s
	...
	for _, fileHeader := range files {
		...
		defer file.Close()
		_, err = apiServer.Controller.FilestoreUpload(apiServer.getRequestContext(req), filepath.Join(fmt.Sprintf("/sessions/%s", session.ID), fileHeader.Filename), file)
		if err != nil {
