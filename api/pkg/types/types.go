...
type Session struct {
	...
	// named manually
	Name          string         `json:"name" gorm:"size:255;index"` // Indexed for search prefix matching
	Created       time.Time      `json:"created"`
	Updated       time.Time      `json:"updated"`
...
type WebsocketEvent struct {
	...
	// incremented once per published patch. The frontend uses it to detect a gap caused
	// by a dropped best-effort NATS message, which is unrecoverable from later deltas,
	// and to resync from a snapshot instead of silently corrupting the content.
