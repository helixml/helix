...
var (
	...
	// `dsh --profile acp` with the Helix patch from /opt/helix/dsh.
	DeepSeekHarnessCommand = "/usr/local/bin/dsh-acp"
	// DeepSeekHarnessHome is $DSH_HOME: profiles, skill discovery and the
	// credential store. Isolated under a helix- prefix so it cannot collide
	// with a harness a user installs themselves.
...
// its whole configuration from desktop/shared/dsh/helix.patch.yml, which
// resolves these variables at boot. Keeping the composition in the image and only
