...
const AgentPreviewCard = styled(Card)<{
  themeConfig: any;
  hasBackgroundImage?: boolean;
}>(({ theme, themeConfig, hasBackgroundImage }) => ({
...
const ConfigCard = styled(Card)<{ themeConfig: any }>(
  ({ theme, themeConfig }) => ({
...
const SecurityWarningCard = styled(Alert)<{ themeConfig: any }>(
  ({ theme, themeConfig }) => ({
...
const LoginIconContainer = styled(Box)<{ themeConfig: any }>(
  ({ themeConfig }) => ({
    width: "80px",
    height: "80px",
    borderRadius: "50%",
...
const AgentIconContainer = styled(Box)<{
  themeConfig: any;
  hasAvatar?: boolean;
}>(({ themeConfig, hasAvatar }) => ({
  flexShrink: 0,
  width: "64px",
  height: "64px",
  borderRadius: "16px",
...
const ImportButton = styled(Button)<{ themeConfig: any }>(
  ({ themeConfig }) => ({
    background: `linear-gradient(135deg, ${themeConfig.tealRoot} 0%, ${themeConfig.magentaRoot} 100%)`,
    border: "none",
    borderRadius: "12px",
...
const ImportAgent: FC = () => {
  ...
  const [modelSubstitutions, setModelSubstitutions] = useState<
    IModelSubstitution[]
  >([]);
  ...
  // Resolve the org for import. This page is not org-scoped (external links
  // can't include an org), so we default to the user's last-selected org from
  // localStorage and let them change it via a dropdown.
  const organizations = account.organizationTools.organizations;
  const [selectedOrgId, setSelectedOrgId] = useState<string>("");

  useEffect(() => {
    if (selectedOrgId || organizations.length === 0) return;
    const storedSlug = localStorage.getItem(SELECTED_ORG_STORAGE_KEY);
    const match = storedSlug
      ? organizations.find((o) => o.name === storedSlug || o.id === storedSlug)
      : undefined;
    const fallback =
      match ||
      organizations.find((o) => (o as any).member !== false) ||
      organizations[0];
    if (fallback?.id) setSelectedOrgId(fallback.id);
    ...
    const parseConfigFromUrl = () => {
      try {
        ...
        if (parsed.yaml_config) {
          ...
          displayYaml = yaml.stringify(parsed.yaml_config, {
            indent: 2,
            lineWidth: 0,
          });
        }
        ...
        setError(
          "Failed to parse agent configuration. The data may be corrupted or in an invalid format.",
        );
        setLoading(false);
      }
    };
  ...
  const getAgentName = () => {
    ...
    if (yamlConfig?.spec?.assistants?.[0]?.name)
      return yamlConfig.spec.assistants[0].name;
  ...
  const getAgentDescription = () => {
    ...
    if (yamlConfig?.spec?.assistants?.[0]?.description)
      return yamlConfig.spec.assistants[0].description;
    if (yamlConfig?.assistants?.[0]?.description)
      return yamlConfig.assistants[0].description;
    // Fall back to system prompt only if no description is available
    if (yamlConfig?.spec?.assistants?.[0]?.system_prompt)
      return yamlConfig.spec.assistants[0].system_prompt;
    if (yamlConfig?.assistants?.[0]?.system_prompt)
      return yamlConfig.assistants[0].system_prompt;
  ...
  const getAgentAvatar = () => {
    ...
    if (yamlConfig?.spec?.assistants?.[0]?.avatar)
      return yamlConfig.spec.assistants[0].avatar;
    if (yamlConfig?.assistants?.[0]?.avatar)
      return yamlConfig.assistants[0].avatar;
    return null;
  };
  ...
  const getAgentImage = () => {
    ...
    if (yamlConfig?.spec?.assistants?.[0]?.image)
      return yamlConfig.spec.assistants[0].image;
    if (yamlConfig?.assistants?.[0]?.image)
      return yamlConfig.assistants[0].image;
    return null;
  };
  ...
  const handleImport = async () => {
    ...
    if (!configData.model_classes || !configData.yaml_config) {
      snackbar.error(
        "Invalid agent configuration format. Please deploy from Launchpad.",
      );
      setImporting(false);
      return;
    }
    ...
    if (result) {
      ...
      const hasSeedZipUrl =
        (actualConfig as any)?.assistants?.[0]?.knowledge?.some(
          (k: any) => k.source?.filestore?.seed_zip_url,
        ) ||
        (actualConfig as any)?.spec?.assistants?.[0]?.knowledge?.some(
          (k: any) => k.source?.filestore?.seed_zip_url,
        );

      const hasOAuthSkills =
        (actualConfig as any)?.assistants?.[0]?.apis?.some(
          (api: any) => api.oauth_provider,
        ) ||
        (actualConfig as any)?.spec?.assistants?.[0]?.apis?.some(
          (api: any) => api.oauth_provider,
        );
  ...
  const navigateToApp = (
    ...
    // Pass org_id explicitly since this page is not org-scoped
    const orgSlug = selectedOrg?.name || selectedOrgId;
    if (hasOAuthSkills) {
      account.orgNavigate(
        "app",
        { app_id: appId, org_id: orgSlug },
        { tab: "skills" },
      );
      snackbar.success(
        "Agent imported successfully! Please review OAuth skills and enable any required providers.",
      );
  ...
  const handleSubstitutionDialogClose = () => {
    ...
    if (pendingNavigation) {
      navigateToApp(
        pendingNavigation.appId,
        pendingNavigation.hasOAuthSkills,
        pendingNavigation.hasSeedZipUrl,
      );
        ...
        <Container maxWidth="sm" sx={{ mt: 8, mb: 4 }}>
          <LoginCard themeConfig={themeConfig}>
            <CardContent sx={{ p: 4, textAlign: "center" }}>
              ...
              >
                To import agents and access your personalized experience, please
              ...
              >
                Don't have an account? You'll be able to create one during the
                sign-in process.
              </Typography>
            </CardContent>
          </LoginCard>
        ...
        <ImportCard themeConfig={themeConfig}>
          <CardContent sx={{ p: 4 }}>
            ...
            >
              ...
              You are about to import an agent configuration from an external
              source. Please ensure you trust the source of this agent before
              proceeding. Review the configuration below carefully.
            </SecurityWarningCard>
              ...
              <Box>
                ...
                >
                  <CardContent sx={{ p: 0, position: "relative", zIndex: 2 }}>
                    ...
                    >
                      {getAgentAvatar() ? (
                        <img src={getAgentAvatar()!} alt={getAgentName()} />
                      ) : (
                        <Bot
                          size={28}
                          color="white"
        ...
        <DialogContent sx={{ p: 3 }}>
          ...
          <List sx={{ bgcolor: themeConfig.darkPanel, borderRadius: 2 }}>
            {modelSubstitutions.map((substitution, index) => (
              <ListItem
                key={index}
                divider={index < modelSubstitutions.length - 1}
              >
                <ListItemText
                  primary={
                    <Typography
                  ...
                  secondary={
                    <Box>
                      ...
                      >
                        ...
                        {substitution.original_model}
                      </Typography>
                      <Typography
                        variant="body2"
                        sx={{ color: themeConfig.tealRoot, mb: 1 }}
                      >
                        <strong>Substituted with:</strong>{" "}
                        {substitution.new_provider} / {substitution.new_model}
                      </Typography>
                      <Typography
                        variant="body2"
                        sx={{
          ...
          <Alert severity="info" sx={{ mt: 3 }}>
            <Typography variant="body2">
              You can change these model selections later in the agent editor if
              needed. The substituted models are compatible alternatives from
              the same performance class, but you should run the tests to ensure
