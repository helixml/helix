...
const MemoizedInteraction = React.memo(
    (props: MemoizedInteractionProps) => {
        ...
        console.log("MemoizedInteraction Debug:", {
            ...
            hasResponseMessage: !!props.interaction.response_message,
        });

        // Show live stream for waiting interactions OR recently completed interactions that were just streaming
        const shouldShowLiveStream = isLive && (props.isOwner || props.isAdmin);
    ...
    (prevProps, nextProps) => {
        ...
        const interactionChanged =
            ...
            prevProps.interaction.output?.length !==
                nextProps.interaction.output?.length ||
            // Check for last_stream_pointer changes (indicates streaming position)
            prevProps.interaction.last_stream_pointer !==
                nextProps.interaction.last_stream_pointer ||
        ...
        const documentIdsChanged =
            !prevProps.session.document_ids ||
            !nextProps.session.document_ids ||
            prevProps.session.document_ids.length !==
                nextProps.session.document_ids.length ||
            (prevProps.session.document_ids.length > 0 &&
                nextProps.session.document_ids.length > 0 &&
                prevProps.session.document_ids[0] !==
                    nextProps.session.document_ids[0]) ||
        ...
        const ragResultsChanged =
            !prevProps.session.rag_results ||
            !nextProps.session.rag_results ||
            prevProps.session.rag_results.length !==
                nextProps.session.rag_results.length ||
            (prevProps.session.rag_results.length > 0 &&
                nextProps.session.rag_results.length > 0 &&
                (prevProps.session.rag_results[0].id !==
                    nextProps.session.rag_results[0].id ||
                    prevProps.session.rag_results[0].timestamp !==
                        nextProps.session.rag_results[0].timestamp));

        // Check if this was the last interaction and we're streaming
        const isLastInteraction =
            prevProps.interaction ===
            prevProps.session.interactions[
                prevProps.session.interactions.length - 1
            ];
        ...
        const lastInteractionNotComplete =
            isLastInteraction &&
            nextProps.interaction.state !== "complete" &&
            nextProps.interaction.state !== "error";
...
const Session: FC<SessionProps> = ({ previewMode = false }) => {
    ...
    const [shareInstructions, setShareInstructions] =
        useState<IShareSessionInstructions>();
    ...
    const [autoScrolledSessionId, setAutoScrolledSessionId] =
        useState<string>("");
    ...
    const initializeVisibleBlocks = useCallback(() => {
        ...
        const startIndex = Math.max(
            0,
            totalInteractions - INTERACTIONS_PER_BLOCK,
        );
    ...
    useEffect(() => {
        ...
        const lastInteraction =
            session.data.interactions[session.data.interactions.length - 1];
    ...
    useEffect(() => {
        ...
        if (container) {
            ...
            return () =>
                container.removeEventListener("scroll", saveScrollOnScroll);
        }
    }, []);
    ...
    const handleInputChange = (event: React.ChangeEvent<HTMLInputElement>) => {
        ...
        requestAnimationFrame(() => {
            ...
            const latency = performance
                .getEntriesByName("input-latency")
                .pop()?.duration;
            console.log(
    ...
    const safeReloadSession = useCallback(
        async (shouldScrollToBottom = false) => {
    ...
    const scrollToBottom = useCallback(() => {
        ...
        if (
            ...
            timeSinceLastScroll >= SCROLL_DEBOUNCE
        ) {
            containerRef.current.scrollTo({
                top: containerRef.current.scrollHeight,
    ...
    const onSend = useCallback(
        async (prompt: string) => {
            ...
            if (
                ...
                newSession = await api.put(
                    `/api/v1/sessions/${session.data?.id}`,
                    formData,
                );
            }
    ...
    const onRegenerate = useCallback(
        async (interactionID: string, message: string) => {
            ...
            if (
                ...
                const interactionIndex = session.data?.interactions?.findIndex(
                ...
                const targetInteraction =
                    session.data?.interactions?.[interactionIndex];
                ...
                const interactionsBeforeTarget =
                    session.data?.interactions?.slice(0, interactionIndex) ||
                    [];
                    ...
                    if (
                        ...
                        if (interaction.prompt_message) {
                            messages.push({
                                ...
                                content: {
                                    content_type:
                                        "text" as TypesMessageContentType,
    ...
    const checkOwnership = useCallback(
        (instructions: IShareSessionInstructions): boolean => {
        ...
        [session.data, account.user, isOwner],
    );

    const proceedToLogin = useCallback(() => {
        localStorage.setItem(
            "shareSessionInstructions",
            JSON.stringify(shareInstructions),
        );
        account.onLogin();
    }, [shareInstructions]);
    ...
    }, [isOwner, account.user, session.data]);
    ...
    const onHandleFilterDocument = useCallback(
        async (docId: string) => {
            // Only pass the filter document handler to the citation component if we have an app ID
            if (!appID) {
                console.warn(
                    "Filter document requested but no appID is available",
                    { docId },
                );
                snackbar.error(
                    "Unable to filter document, no app ID available in standalone session view",
                );
                return;
            }
            ...
            if (result.status !== 200) {
                snackbar.error(
                    `Unable to filter document, error from API: ${result.statusText}`,
                );
                return;
            }
            const filterAction = result.data?.data?.find(
                (item) =>
                    item.value?.includes(docId) &&
                    item.action_label?.toLowerCase().includes("filter"),
            );
            if (!filterAction) {
    ...
    const sessionData = useMemo(() => {
        if (!session.data) {
            console.log("SessionData Debug: session.data is null/undefined");
            return null;
        }
        ...
        console.log("SessionData Debug:", {
            sessionId: session.data.id,
            interactionsCount: session.data.interactions?.length || 0,
            lastInteractionId:
                session.data.interactions?.[
                    session.data.interactions.length - 1
                ]?.id,
    ...
    const addBlocksAbove = useCallback(() => {
        ...
        const newStartIndex = Math.max(
            0,
            firstBlock.startIndex - INTERACTIONS_PER_BLOCK,
        );
    ...
    }, [session.data?.interactions, visibleBlocks, isLoadingBlock]);
    ...
    const renderInteractions = useCallback(() => {
        ...
        const hasMoreAbove =
            visibleBlocks.length > 0 && visibleBlocks[0].startIndex > 0;
            ...
            >
                ...
                >
                    {visibleBlocks.map((block) => {
                        ...
                        const blockInteractions = memoizedInteractions.slice(
                            block.startIndex,
                            block.endIndex,
                        );
                        ...
                        return (
                            ...
                            >
                                {blockInteractions.map((interaction, index) => {
                                    ...
                                    const isLastInteraction =
                                        absoluteIndex ===
                                        memoizedInteractions.length - 1;
                                    const isOwner =
                                        account.user?.id === sessionData.owner;
    ...
    const activeAssistant =
        appID && apps.app && assistantID
            ? getAssistant(apps.app, assistantID)
            : null;
    ...
    useWebsocket(sessionID, (parsedData) => {
        if (
            parsedData.type === WEBSOCKET_EVENT_TYPE_SESSION_UPDATE &&
            parsedData.session
        ) {
            ...
            console.log("WebSocket session update received:", {
                ...
                interactionsCount: newSession.interactions?.length || 0,
                lastInteractionState:
                    newSession.interactions?.[
                        newSession.interactions.length - 1
                    ]?.state,
                lastInteractionResponseLength:
                    newSession.interactions?.[
                        newSession.interactions.length - 1
                    ]?.response_message?.length || 0,
                currentSessionDataExists: !!session.data,
                currentInteractionCount:
                    session.data?.interactions?.length || 0,
            });

            // Validate the WebSocket session data before replacing current session
            // This prevents blank screen when incomplete session updates arrive
            const newInteractionCount = newSession.interactions?.length || 0;

            // Always reject session updates with 0 interactions - these are invalid
            if (newInteractionCount === 0) {
                console.log(
                    "Rejecting WebSocket session update - 0 interactions (invalid)",
                    {
                        sessionId: newSession.id,
                        newInteractionCount,
                    },
            ...
            // If we have current session data, don't accept updates with fewer interactions
            if (session.data && session.data.interactions) {
                const currentInteractionCount =
                    session.data.interactions.length;

                if (newInteractionCount < currentInteractionCount) {
                    console.log(
                        "Rejecting WebSocket session update - fewer interactions than current",
                        {
                            current: currentInteractionCount,
                            new: newInteractionCount,
                        },
                    );
                    return;
                }
    ...
    useEffect(() => {
        ...
        if (
            ...
            lastInteraction.state == TypesInteractionState.InteractionStateError
        )
            return;
        ...
        return () => clearTimeout(timer);
    }, [session.data, safeReloadSession]);
    ...
    return (
        ...
        >
                                    ...
                                    <Cell flexGrow={1}>
                                        ...
                                        >
                                            ...
                                            >
                                                <textarea
                                                    ref={
                                                        textFieldRef as React.RefObject<HTMLTextAreaElement>
                                                    }
                                                    value={inputValue}
                                                    onChange={(e) =>
                                                        setInputValue(
            ...
            {showLoginWindow && (
                ...
                >
                    <Typography gutterBottom>
                        You can login with your Google account or your
                        organization's SSO provider.
                    </Typography>
                    <Typography>
                        This session will be cloned into your account and you
            ...
            {showCloneWindow && (
                ...
                >
                    <Typography>
                        This session will be cloned into your account where you
                        will be able to continue this session.
                    </Typography>
                </Window>
            )}
                    ...
                    <Box sx={{ p: 2, width: "100%" }}>
                        ...
                        {account.serverConfig.eval_user_id && (
                            <Row sx={{ mt: 2 }}>
                                <Cell grow>
                                    <Typography>
                                        Clone the session into the evals
                                        account:
                                    </Typography>
                                </Cell>
                                <Cell
