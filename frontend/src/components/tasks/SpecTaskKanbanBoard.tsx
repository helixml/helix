...
const SpecTaskKanbanBoard: React.FC<SpecTaskKanbanBoardProps> = ({
  ...
  const handleAssignAgent = useCallback(
    async (taskId: string, agentType: string) => {
      try {
        console.log(
          "Starting agent session for task:",
          taskId,
          "with agent:",
          agentType,
        );
        ...
        const response = await api.post(`/api/v1/agents/work`, {
          name:
            tasks.find((t) => t.id === taskId)?.name || "Spec Generation Task",
        ...
        if (response && response.data) {
          ...
          try {
            ...
            const specTasks: SpecTask[] = Array.isArray(tasksData)
              ? tasksData
              : [];

            // Use consistent phase mapping helper
            const enhancedTasks: SpecTaskWithExtras[] = specTasks.map(
              (task) => {
            ...
            console.error(
              "Failed to reload tasks after agent assignment:",
              err,
            );
          }
        }
