...
const useAgentActivityCheck = (
  ...
  const needsAttention =
    !isActive && sessionUpdatedAt !== lastSeenTimestamp && !!sessionUpdatedAt;
...
const countLeafNodes = (root: PanelNode | null): number => {
  ...
  return (
    root.children?.reduce((sum, child) => sum + countLeafNodes(child), 0) || 0
  );
};
...
const taskHasSpecReview = (task: TypesSpecTask): boolean => {
  ...
  return (
    statusesWithSpec.includes(status) ||
...
const PanelTab: React.FC<PanelTabProps> = ({
  ...
  const { data: refreshedTask } = useSpecTask(
    tab.type === "task" ? tab.id : "",
    {
  ...
  const {
    ...
    needsAttention,
    markAsSeen,
  } = useAgentActivityCheck(displayTask?.session_updated_at, hasSession);
  ...
  const tooltipContent = useMemo(() => {
    ...
    return (
      <Box sx={{ p: 0.5 }}>
        ...
        {titleHistory
          .slice(0, 5)
          .map((entry: TypesTitleHistoryEntry, idx: number) => (
            <Box
              key={idx}
  ...
  return (
    ...
    >
      ...
      >
        ...
        {/* Task number prefix for task tabs */}
        {tab.type === "task" &&
          displayTask?.task_number &&
          displayTask.task_number > 0 && (
            <Typography
              variant="body2"
              noWrap
              sx={{
            ...
            >
              #{String(displayTask.task_number).padStart(6, "0")}
            </Typography>
          )}
...
const TaskPanel: React.FC<TaskPanelProps> = ({
  ...
  const activeTask = activeTab
    ? tasks.find((t) => t.id === activeTab.id) || activeTab.task
    : null;
  ...
  const handleReviewSpec = async () => {
    ...
    try {
      const response = await api
        .getApiClient()
        .v1SpecTasksDesignReviewsDetail(activeTask.id);
  ...
  const handleApproveImplementation = async () => {
    ...
    try {
      ...
      snackbar.error(
        err?.response?.data?.message || "Failed to approve implementation",
      );
    } finally {
      setIsActioning(false);
    }
  ...
  const performArchive = async () => {
    ...
    try {
      await api
        .getApiClient()
        .v1SpecTasksArchivePartialUpdate(activeTask.id, { archived: true });
  ...
  const handleDrop = (e: React.DragEvent) => {
    ...
    if (dragOverEdge) {
      ...
      const direction =
        dragOverEdge === "left" || dragOverEdge === "right"
          ? "horizontal"
          : "vertical";
                  ...
                  >
                    <ListItemIcon>
                      <DesktopIcon
                        sx={{
                          fontSize: 16,
                          color: alreadyOpen ? "text.disabled" : "success.main",
                        }}
                      />
                    </ListItemIcon>
                    <ListItemText
...
const getWorkspaceStateKey = (projectId: string) =>
  `${WORKSPACE_STATE_KEY_PREFIX}${projectId}`;
...
const TabsView: React.FC<TabsViewProps> = ({
  ...
  useEffect(() => {
    ...
    localStorage.setItem(
      getWorkspaceStateKey(projectId),
  ...
  useEffect(() => {
    ...
    if (initialTaskId) {
      ...
      if (taskToOpen) {
        if (restoredRoot) {
          ...
          const alreadyOpen = allLeaves.some((leaf) =>
            leaf.tabs?.some((t) => t.id === initialTaskId),
          );
          if (!alreadyOpen) {
            ...
            restoredRoot = updateNodeInTree(
              restoredRoot,
              allLeaves.find((l) => l.tabs?.some((t) => t.id === initialTaskId))
                ?.id || "",
    ...
    if (initialDesktopId) {
      ...
      if (restoredRoot) {
        const allLeaves = getAllLeafNodes(restoredRoot);
        const alreadyOpen = allLeaves.some((leaf) =>
          leaf.tabs?.some((t) => t.id === desktopTabId),
        );
        if (!alreadyOpen) {
          ...
          restoredRoot = updateNodeInTree(
            restoredRoot,
            allLeaves.find((l) => l.tabs?.some((t) => t.id === desktopTabId))
              ?.id || "",
  ...
  useEffect(() => {
    ...
    if (existingLeaf) {
      ...
      setRootNode((prev) =>
        prev
          ? updateNodeInTree(prev, existingLeaf.id, (node) => ({
              ...node,
    ...
    const emptyLeaf = allLeaves.find(
      (leaf) => !leaf.tabs || leaf.tabs.length === 0,
    );
    if (emptyLeaf) {
      // Add to empty pane
      setRootNode((prev) =>
        prev
          ? updateNodeInTree(prev, emptyLeaf.id, (node) => ({
              ...node,
              tabs: [
    ...
    if (firstLeaf) {
      ...
      setRootNode((prev) =>
        prev ? replaceNodeInTree(prev, firstLeaf.id, newSplit) : prev,
      );
    }
  ...
  useEffect(() => {
    if (!initialized || !rootNode) return;
    if (!initialDesktopId || initialDesktopId === lastOpenedDesktopRef.current)
      return;
  ...
  useEffect(() => {
    ...
    const allLeaves = getAllLeafNodes(rootNode);
    const existingLeaf = allLeaves.find((leaf) =>
      leaf.tabs?.some((t) => t.id === reviewTabId),
    );
    if (existingLeaf) {
      // Just activate it
      setRootNode((prev) =>
  ...
  const handleSplitPanel = useCallback(
    (
      ...
      setRootNode((prev) => {
        ...
        if (taskId && fromPanelId && fromPanelId !== panelId) {
          ...
          if (sourcePanel && sourcePanel.type === "leaf") {
            ...
            if (tabToMove) {
              ...
              updatedRoot = updateNodeInTree(prev, fromPanelId, (node) => ({
                ...
                activeTabId:
                  newSourceTabs.length > 0
                    ? newSourceTabs.some((t) => t.id === node.activeTabId)
                      ? node.activeTabId
                      : newSourceTabs[0].id
                    : null,
              }));
              ...
              if (
                newSourceTabs.length === 0 &&
                countLeafNodes(updatedRoot) > 1
              ) {
                updatedRoot =
                  removeNodeFromTree(updatedRoot, fromPanelId) || updatedRoot;
              }
            }
          }
        } else if (taskId) {
        ...
        const newSplit = createSplitNode(direction, [
          updatedTargetLeaf,
          newLeaf,
        ]);
  ...
  const handleDropTab = useCallback(
    (targetPanelId: string, tabId: string, fromPanelId: string) => {
      setRootNode((prev) => {
        if (!prev) return prev;
        ...
        if (sourcePanel.type !== "leaf" || targetPanel.type !== "leaf")
          return prev;
  ...
  const handleTaskCreated = useCallback(
    (panelId: string, task: TypesSpecTask) => {
      if (!task.id) return;
      setRootNode((prev) => {
        if (!prev) return prev;
        return updateNodeInTree(prev, panelId, (node) => ({
      ...
      setRootNode((prev) => {
        ...
        for (const leaf of allLeaves) {
          if (leaf.tabs?.some((t) => t.id === tabId)) {
            // Activate it
            return updateNodeInTree(prev, leaf.id, (node) => ({
              ...node,
              activeTabId: tabId,
            }));
          }
        ...
        if (sourcePanelId) {
          ...
          if (sourcePanel && sourcePanel.type === "leaf") {
            ...
            const newSplit = createSplitNode("vertical", [
              sourcePanel,
              newReviewLeaf,
            ]);
  ...
  const renderPanelNode = (node: PanelNode): React.ReactNode => {
    ...
    return (
      <PanelGroup
        orientation={node.direction || "horizontal"}
        style={{ height: "100%" }}
      >
        {children.map((child, index) => (
          <React.Fragment key={child.id}>
            {index > 0 && (
              <ResizeHandle direction={node.direction || "horizontal"} />
            )}
