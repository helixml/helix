...
import { useModelMemoryEstimation, formatMemorySize } from '../../services/memoryEstimationService'
...
const MemoryEstimateCell: FC<MemoryEstimateCellProps> = ({ model }) => {
  ...
  const { data: memoryEstimate, isLoading, error } = useModelMemoryEstimation(
    model.id || '',
    selectedGPUCount,
    { enabled: !!model.id }
  )
  ...
  const getMemoryEstimateContent = () => {
    ...
    if (error || !memoryEstimate || memoryEstimate.error) {
      return (
        <Tooltip title={error?.message || memoryEstimate?.error || 'Failed to estimate'}>
          <Typography variant="body2" color="text.secondary">
            -
          </Typography>
        </Tooltip>
      ...
      <Box display="flex" alignItems="center" gap={1}>
        ...
        <Tooltip title={
          <Box>
            ...
            <Typography variant="body2">KV Cache: {formatMemorySize(estimate.kv_cache || 0)}</Typography>
            <Typography variant="body2">Graph: {formatMemorySize(estimate.graph || estimate.graph_mem || 0)}</Typography>
            <Typography variant="body2">Architecture: {estimate.architecture || 'Unknown'}</Typography>
            {estimate.requires_fallback && (
              <Typography variant="body2" color="info.main">Supports CPU fallback</Typography>
            )}
          </Box>
        }>
      ...
      <Box display="flex" alignItems="center" gap={1}>
        ...
        >
          ...
          <Typography variant="caption" ml={0.5}>
            {selectedGPUCount}GPU{selectedGPUCount > 1 ? 's' : ''}
          </Typography>
        </IconButton>
