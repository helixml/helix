...
const AppLogsTable: FC<AppLogsTableProps> = ({ appId }) => {
  ...
  const prepareChartData = (usageData: TypesUsersAggregatedUsageMetric[]) => {
    if (usageLoading) return { xAxis: [], series: [], totalValue: 0 };
  ...
  const requestChartData = useMemo(() => prepareRequestChartData(usageData as TypesUsersAggregatedUsageMetric[]), [usageData, selectedPeriod, usageLoading]);
  const costsChartData = useMemo(() => prepareCostsChartData(usageData as TypesUsersAggregatedUsageMetric[]), [usageData, selectedPeriod, usageLoading]);
