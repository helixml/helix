		...
		<div>
			<Typography
				...
				sx={{
					...
					"&:hover": {
						backgroundColor:
							theme.palette.mode === "light" ? "#efefef" : "#13132b",
					},
					cursor: "pointer",
				}}>
				&nbsp;&nbsp;{model}{" "}
				<KeyboardArrowDownIcon sx={{ position: "relative", top: "5px" }} />
				&nbsp;
			</Typography>
			<Box component="span" sx={{ display: "flex", alignItems: "center" }}>
				<Menu
					...
					<MenuItem
						sx={{ fontSize: "large" }}
						onClick={() => {
							updateModel("Helix 3.5")
							setModelMenuAnchorEl(null)
						}}>
						Helix 3.5 &nbsp;{" "}
						<small>(Llama3-8B, fast and good for everyday tasks)</small>
					</MenuItem>
					<MenuItem
						sx={{ fontSize: "large" }}
						onClick={() => {
					...
					<MenuItem
						sx={{ fontSize: "large" }}
						onClick={() => {
							updateModel("Helix Code")
							setModelMenuAnchorEl(null)
						}}>
						Helix Code &nbsp;{" "}
						<small>(CodeLlama 70B from Meta, better than GPT-4 at code)</small>
					</MenuItem>
					<MenuItem
						sx={{ fontSize: "large" }}
						onClick={() => {
							updateModel("Helix JSON")
							setModelMenuAnchorEl(null)
						}}>
						Helix JSON &nbsp;{" "}
						<small>
							(Nous Hermes 2 Pro 7B, for function calling & JSON output)
						</small>
					</MenuItem>
					<MenuItem
						sx={{ fontSize: "large" }}
						onClick={() => {
							updateModel("Helix Small")
							setModelMenuAnchorEl(null)
						}}>
						Helix Small &nbsp;{" "}
						<small>(Phi-3 Mini 3.8B, fast and memory efficient)</small>
					</MenuItem>
					{/* TODO: Vision model */}
				</Menu>
			</Box>
