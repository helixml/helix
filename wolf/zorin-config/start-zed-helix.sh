#!/bin/bash
# Startup script for Zed editor connected to Helix controlplane (GNOME/Zorin version)
set -e
...
echo "========================================="
echo "Helix Agent Startup (Zorin) - $(date)"
echo "========================================="
echo ""
...
# NOTE: Zed state symlinks are already created by startup-app.sh BEFORE GNOME starts
...
if [ -n "$HELIX_PRIMARY_REPO_NAME" ]; then
    ...
    if [ -d "$PRIMARY_REPO_PATH/.git" ]; then
        ...
        if [ -f "$SCRIPT_DIR/helix-specs-create.sh" ]; then
            ...
            echo "  ⚠️  helix-specs-create.sh not found at $SCRIPT_DIR, skipping worktree setup"
        fi
    else
...
WRAPPER_EOF
    ...
    # Use gnome-terminal for Zorin/GNOME (vs kitty for Sway)
