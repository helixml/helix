#!/bin/bash
# Startup script for Zed editor connected to Helix controlplane (XFCE version)
set -e
...
# XFCE supports both Wayland and X11 - prefer Wayland if available
if [ -z "$WAYLAND_DISPLAY" ] && [ -z "$DISPLAY" ]; then
    echo "ERROR: Neither WAYLAND_DISPLAY nor DISPLAY is set! Cannot start Zed."
    exit 1
fi
...
if [ -n "$WAYLAND_DISPLAY" ]; then
    ...
    echo "Using X11 backend (DISPLAY=$DISPLAY)"
fi
