        ...
        if [ -d "$CLONE_DIR/.git" ]; then
            ...
            if git -C "$CLONE_DIR" fetch origin 2>&1; then
                ...
                if git -C "$CLONE_DIR" checkout "origin/$DEFAULT_BRANCH" -- .helix/startup.sh 2>/dev/null; then
                    ...
                    echo "  No startup script changes to pull (or file doesn't exist)"
                fi
            else
                echo "  Warning: Failed to fetch latest changes (continuing with cached version)"
            fi
            continue
        fi
...
# Launch ACP log viewer in xfce4-terminal (for debugging agent issues)
...
if [ "$SHOW_ACP_DEBUG_LOGS" = "true" ] || [ -n "$HELIX_DEBUG" ]; then
    echo "Starting ACP log viewer in xfce4-terminal..."
    xfce4-terminal --title="ACP Agent Logs" \
          -e 'bash -c "
              echo \"═══════════════════════════════════════════════════════════════\"
